...
public class NfcImpl implements Nfc {
  ...
  /**
   * Activity that is in foreground and is used to enable / disable NFC reader mode operations. Can
   * be updated when activity associated with web page is changed. @see #setActivity
   */
  ...
  /**
   * Object that contains data that was passed to method #push(NdefMessage message, NdefWriteOptions
   * options, Push_Response callback)
   *
  ...
  /**
   * Object that contains the callback that was passed to method #makeReadOnly(MakeReadOnly_Response
   * callback)
   *
  ...
  public NfcImpl(int hostId, NfcDelegate delegate, InterfaceRequest<Nfc> request) {
    ...
    if (!mHasPermission) {
      ...
      mNfcManager =
          (NfcManager) ContextUtils.getApplicationContext().getSystemService(Context.NFC_SERVICE);
      if (mNfcManager == null) {
    ...
    mVibrator =
        (Vibrator) ContextUtils.getApplicationContext().getSystemService(Context.VIBRATOR_SERVICE);
  }
  ...
  /**
   * Forces the Mojo connection to this object to be closed. This will trigger a call to close() so
   * that pending NFC operations are canceled.
   */
  ...
  public void push(NdefMessage message, NdefWriteOptions options, Push_Response callback) {
    ...
    if (!NdefMessageValidator.isValid(message)) {
      callback.call(
          createError(
              NdefErrorType.INVALID_MESSAGE, "Cannot push the message because it's invalid."));
      return;
    }
    ...
    if (mPendingPushOperation != null) {
      mPendingPushOperation.complete(
          createError(
              NdefErrorType.OPERATION_CANCELLED, "Push is cancelled due to a new push request."));
    }
  ...
  /**
   * When NdefMessages that are found when NFC device is within proximity, it is passed to NfcClient
  ...
  private static class PendingPushOperation {
     ...
     *
     * @param error should be null when operation is completed successfully, otherwise, error object
     *     with corresponding NdefErrorType should be provided.
     */
  ...
  /**
   * Handles completion of pending push operation, completes push operation. On error, invalidates
   * #mTagHandler.
   */
    ...
    try {
      ...
      if (!mPendingPushOperation.ndefWriteOptions.overwrite && !mTagHandler.canAlwaysOverwrite()) {
        ...
        pendingPushOperationCompleted(
            createError(
                NdefErrorType.NOT_ALLOWED, "NDEFWriteOptions#overwrite does not allow overwrite."));
        return;
      }
      ...
      pendingPushOperationCompleted(
          createError(
              NdefErrorType.IO_ERROR, "Failed to write due to an IO error: " + e.getMessage()));
    }
  }

  /**
   * Handles completion of pending make read-only operation, completes make read-only operation. On
   * error, invalidates #mTagHandler.
   */
  ...
  /**
   * Checks whether there is a #mPendingMakeReadOnlyOperation and make NFC tag read-only. In case of
   * exception calls pendingMakeReadOnlyOperationCompleted() with appropriate error object.
   */
  ...
  private void processPendingWatchOperations() {
    ...
    try {
      ...
      notifyErrorToAllWatchers(
          createError(
              NdefErrorType.IO_ERROR, "Failed to read because the tag is lost: " + e.getMessage()));
    } catch (FormatException | IllegalStateException | IOException | SecurityException e) {
      Log.w(TAG, "Cannot read data from NFC tag. IO_ERROR: " + e.getMessage());
      notifyErrorToAllWatchers(
          createError(
              NdefErrorType.IO_ERROR, "Failed to read due to an IO error: " + e.getMessage()));
    }
  }
