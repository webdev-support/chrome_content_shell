...
final class ChromeBluetoothAdapter extends BroadcastReceiver {
  ...
  /**
   * @return true if Chromium has permission to scan for Bluetooth devices and location services are
   *     on.
   */
  ...
  private void registerBroadcastReceiver() {
    if (mAdapter != null) {
      ContextUtils.registerProtectedBroadcastReceiver(
          mAdapter.getContext(), this, new IntentFilter(BluetoothAdapter.ACTION_STATE_CHANGED));
    }
  }
  ...
  private class ScanCallback extends Wrappers.ScanCallbackWrapper {
    ...
    public void onScanResult(int callbackType, Wrappers.ScanResultWrapper result) {
      ...
      SparseArray<byte[]> manufacturerData = result.getScanRecord_getManufacturerSpecificData();
