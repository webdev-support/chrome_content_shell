...
public interface WebContents extends Parcelable {
   ...
   *
   * <p>Initialize various content objects of {@link WebContents} lifetime.
   *
   ...
   *
   * <p>Note: This is a temporary workaround for Chrome until it can clean up Observers directly.
  ...
  /**
   * @return The {@link ViewAndroidDelegate} from which to get the container view. This can be null.
   */
  @Nullable
  ...
  /**
   * Removes the native WebContents' reference to this object. This is used when we want to destroy
   * this object without destroying its native counterpart.
   */
  @Deprecated
  ...
  /**
   * @return Whether the focused frame element in this WebContents is editable. Will be false if the
   *     WebContents does not have focus.
   */
  boolean isFocusedElementEditable();

  /**
   * @return The frame associated with the id. Will be null if the ID does not correspond to a live
   *     RenderFrameHost.
   */
  @Nullable
  ...
  /**
   * ChildProcessImportance on Android allows controls of the renderer process bindings independent
   * of visibility. Note this does not affect importance of subframe processes or main frames
  ...
  /**
   * Suspends all media players for this WebContents. Note: There may still be activities generating
   * audio, so setAudioMuted() should also be called to ensure all audible activity is silenced.
   */
  ...
  /**
   * Adjusts the selection starting and ending points by the given amount. A negative amount moves
   * the selection towards the beginning of the document, a positive amount moves the selection
  ...
  /**
   * Gets the last committed URL. It represents the current page that is displayed in this
   * WebContents. It represents the current security context.
   *
  ...
  /**
   * Injects the passed Javascript code in the current page and evaluates it. If a result is
   * required, pass in a callback.
   *
   * <p>It is not possible to use this method to evaluate JavaScript on web content, only on WebUI
   * pages.
   *
   * @param script The Javascript to execute.
   * @param callback The callback to be fired off when a result is ready. The script's result will
   *     be json encoded and passed as the parameter, and the call will be made on the main thread.
   ...
   * @param targetOrigin The target origin. If the target origin is a "*" or a empty string, it
   *     indicates a wildcard target origin.
   * @param ports The sent message ports, if any. Pass null if there is no message ports to pass.
   */
  ...
  /**
   * Creates a message channel for sending postMessage requests and returns the ports for each end
   * of the channel.
   *
  ...
  /**
   * Returns whether the initial empty page has been accessed by a script from another page. Always
   * false after the first commit.
   *
  ...
  /**
   * Initiate extraction of text, HTML, and other information for clipping puposes (smart clip) from
   * the rectangle area defined by starting positions (x and y), and width and height.
   */
  ...
  /**
   * Sends a request to download the given image {@link url}. This method delegates the call to the
   * downloadImage() method of native WebContents.
   *
   ...
   *     max_bitmap_size} are filtered out from the bitmap results. If there are no bitmap results
   *     <= {@link max_bitmap_size}, the smallest bitmap is resized to {@link max_bitmap_size} and
   *     is the only result. A {@link max_bitmap_size} of 0 means unlimited.
  ...
  /**
   * Whether the WebContents has an active fullscreen video with native or custom controls. The
   * WebContents must be fullscreen when this method is called. Fullscreen videos may take a moment
   * to register.
   */
  boolean hasActiveEffectivelyFullscreenVideo();

  /**
   * Whether the WebContents is allowed to enter Picture-in-Picture when it has an active fullscreen
   * video with native or custom controls.
   */
  boolean isPictureInPictureAllowedForFullscreenVideo();

  /**
   * Gets a Rect containing the size of the currently playing fullscreen video. The position of the
   * rectangle is meaningless. Will return null if there is no such video. Fullscreen videos may
   ...
   * bottom controls height, top controls min-height, and bottom controls min-height will call this.
   * Min-height is the minimum visible height the controls can have.
   */
  ...
  /**
   * This function checks all frames in this WebContents (not just the main frame) and returns true
   * if at least one frame has either a beforeunload or an unload/pagehide/visibilitychange handler.
   */
  ...
  public ChromiumWebView getWebView();

  public void setWebView(ChromiumWebView webView);
}
