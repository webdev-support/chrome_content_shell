...
class MidiManagerAndroid {
  ...
  /**
   * True is this object is stopped. This is needed because MidiManagerAndroid functions are called
   * from the IO thread but callbacks are called on the UI thread (because the IO thread doesn't
   * have a Looper). We need to protect each native function call with a synchronized block that
  ...
  private MidiManagerAndroid(long nativeManagerPointer) {
    ...
    mManager =
        (MidiManager) ContextUtils.getApplicationContext().getSystemService(Context.MIDI_SERVICE);
  ...
  /** Initializes this object. This function must be called right after creation. */
  @CalledByNative
  void initialize() {
    if (mManager == null) {
  ...
  private synchronized void onDeviceOpened(MidiDevice device, MidiDeviceInfo info) {
    ...
    if (!mIsInitialized && mPendingDevices.isEmpty()) {
      MidiManagerAndroidJni.get()
          .onInitialized(mNativeManagerPointer, mDevices.toArray(new MidiDeviceAndroid[0]));
      mIsInitialized = true;
    }
  }
