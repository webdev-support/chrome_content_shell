...
public interface Interface extends ConnectionErrorHandler, Closeable {
  ...
  public interface Proxy extends Interface {
    ...
    public interface Handler extends Closeable {
      ...
      /**
       * Unbinds the proxy and passes the handle. Can return null if the proxy is not bound or if
       * the proxy is not over a message pipe.
       */
      ...
      /**
       * Queries the max version that the remote side supports. On completion, the result will be
       * returned as the input of |callback|. The version number of this interface pointer will also
       * be updated.
       */
      public void queryVersion(QueryVersionCallback callback);

      /**
       * If the remote side doesn't support the specified version, it will close its end of the
       * message pipe asynchronously. The call does nothing if |version| is no greater than
       * getVersion().
       *
       * <p>If you make a call to requireVersion() with a version number X which is not supported by
       * the remote side, it is guaranteed that all calls to the interface methods after
            ...
            new InterfaceControlMessagesHelper.SendRunMessageCallback() {
              ...
              public void call(RunResponseMessageParams response) {
                if (response.output != null
                    && response.output.which() == RunOutput.Tag.QueryVersionResult) {
  ...
  /**
   * A {@link MessageReceiverWithResponder} implementation that forwards all calls to the thread the
   * ThreadSafeForwarder was created.
   */
  ...
  abstract class Manager<I extends Interface, P extends Proxy> {
    ...
    /**
     * Returns the name of the interface. This is an opaque (but human readable) identifier used by
     * the service provider to identify services.
     */
    ...
    /**
     * Binds the given implementation to the handle. Returns the router that owns the implementation
     * and connection handle, which can be used to close the binding if necessary. If the router
     * (and by consequence the handle) is intentionally leaked it will close itself when the
     * connection handle is closed and the proxy receives the connection error.
     */
    ...
    /**
     * Returns a Proxy that will send messages to the given |handle|. This implies that the other
     * end of the handle must be bound to an implementation of the interface.
     */
    ...
    /**
     * Constructs a new |InterfaceRequest| for the interface. This method returns a Pair where the
     * first element is a proxy, and the second element is the request. The proxy can be used
     * immediately.
     */
    ...
    /**
     * Constructs a thread-safe Proxy forwarding the calls to the given message receiver. All calls
     * can be performed from any thread and are posted to the {@link Executor} that is associated
     * with the thread on which this method was called on.
     *
     * <p>The original Proxy object is unbound.
     */
    public final P buildThreadSafeProxy(P proxy) {
      ...
      proxy =
          buildProxy(core, new ThreadSafeForwarder(core, new AutoCloseableRouter(core, router)));
      DelegatingConnectionErrorHandler handlers = new DelegatingConnectionErrorHandler();
      handlers.addConnectionErrorHandler(proxy);
      router.setErrorHandler(handlers);
      router.start();
