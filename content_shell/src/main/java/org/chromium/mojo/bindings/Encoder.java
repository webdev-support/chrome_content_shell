...
public class Encoder {
  ...
  private static class EncoderState {
     ...
     * @param bufferSize A hint on the size of the message. Used to build the initial byte buffer.
     */
    private EncoderState(Core core, int bufferSize) {
      assert bufferSize % BindingsHelper.ALIGNMENT == 0;
      this.core = core;
      byteBuffer = ByteBuffer.allocateDirect(bufferSize > 0 ? bufferSize : INITIAL_BUFFER_SIZE);
   ...
   * Constructor.
   *
   * @param core the |Core| implementation used to generate handles. Only used if the data structure
  ...
  public <T extends Interface> void encode(
    ...
    Pair<MessagePipeHandle, MessagePipeHandle> handles = mEncoderState.core.createMessagePipe(null);
  ...
  public void encode(
      AssociatedInterfaceNotSupported[] v, int offset, int arrayNullability, int expectedLength) {}
  ...
  /**
   * Encodes a <code>null</code> pointer iff the object is nullable, raises an exception otherwise.
   */
