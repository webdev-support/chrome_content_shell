...
public class FeatureList {
  ...
  /**
   * @return Whether the native FeatureList has been initialized. If this method returns false, none
   *     of the methods in this class that require native access should be called (except in tests
   *     if test features have been set).
   */
  ...
  /**
   * Sets the feature flags and field trial parameters to use in JUnit tests, since native calls are
   * not available there.
   */
  @VisibleForTesting
  ...
  public static void mergeTestValues(@NonNull TestValues testValuesToMerge, boolean replace) {
    ...
    for (Map.Entry<String, Map<String, String>> e :
      ...
      Map<String, String> fieldTrialParamsForFeature =
          newTestValues.mFieldTrialParams.get(featureName);
   ...
   * @return The test values set for the parameter, or null if no test values have been set (if test
   *     values were set for other features, an empty Map will be returned, not null).
   */
