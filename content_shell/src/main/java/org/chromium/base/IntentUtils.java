...
public class IntentUtils {
   ...
   *     <p>Creates a temporary copy of the extra Bundle, which is required as
  ...
  public static boolean intentTargetsSelf(Context context, Intent intent) {
    ...
    boolean matchesComponent =
        hasComponent && context.getPackageName().equals(intent.getComponent().getPackageName());
  ...
  /**
   * Sets TRUSTED_APPLICATION_CODE_EXTRA on the provided intent to identify it as coming from a
   * trusted source.
   *
   * @param intent An Intent that targets either current package, or explicitly targets a component
