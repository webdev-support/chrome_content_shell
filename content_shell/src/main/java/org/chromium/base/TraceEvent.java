 ...
 *
 * <p>It is OK to use tracing before the native library has loaded, in a slightly restricted
 * fashion.
 *
...
public class TraceEvent implements AutoCloseable {
  ...
  static class BasicLooperMonitor implements Printer {
     ...
     *
     * <p>">>>>> Dispatching to (TARGET) {HASH_CODE} TARGET_NAME: WHAT"
     *
     * <p>and
     *
     * <p>"<<<<< Finished to (TARGET) {HASH_CODE} TARGET_NAME".
     *
     * <p>This has been the case since at least 2009 (Donut). This function extracts the TARGET part
     * of the message.
     */
  ...
  /**
   * A class that records, traces and logs statistics about the UI thead's Looper. The output of
   * this class can be used in a number of interesting ways:
   *
   * <p>
   *
   * <ol>
   *   <li>When using chrometrace, there will be a near-continuous line of measurements showing both
   *       event dispatches as well as idles;
   *   <li>Logging messages are output for events that run too long on the event dispatcher, making
   *       it easy to identify problematic areas;
   *   <li>Statistics are output whenever there is an idle after a non-trivial amount of activity,
   *       allowing information to be gathered about task density and execution cadence on the
   *       Looper;
   * </ol>
   *
   * <p>The class attaches itself as an idle handler to the main Looper, and monitors the execution
   * of events and idle notifications. Task counters accumulate between idle notifications and get
   * reset when a new idle notification is received.
   */
   ...
   *
   * <p>Note that if tracing is not enabled, this will not result in allocating an object.
   *
   ...
   *
   * @param readCommandLine If true, also check command line flags to see whether tracing should be
   *     turned on.
   */
   ...
   *     checking if TraceEvent is enabled.
   */
   ...
   * `isColdStartup` with the 'android_webview.timeline' category starting at `startTimeMs` with the
   * duration of `durationMs`.
   */
  ...
  /**
   * Snapshots the view hierarchy state on the main thread and then finishes emitting a trace event
   * on the threadpool.
   */
  ...
  /**
   * A class that periodically dumps the view hierarchy of all running activities of the app to the
   * trace. Enabled/disabled via the disabled-by-default-android_view_hierarchy trace category.
   *
   * <p>The class registers itself as an idle handler, so that it can run when there are no other
   * tasks in the queue (but not more often than once a second). When the queue is idle, it calls
   * the initViewHierarchyDump() native function which in turn calls the
   ...
   * activity. Altogether, the call sequence is as follows: ViewHierarchyDumper.queueIdle() ->
   * JNI#initViewHierarchyDump() -> TraceEvent.dumpViewHierarchy() -> JNI#startActivityDump() ->
   * ViewHierarchyDumper.dumpView() -> JNI#addViewDump()
   */
