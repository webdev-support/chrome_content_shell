...
public class ChildProcessLauncher {
  ...
  public abstract static class Delegate {
    /**
     * Called when the launcher is about to start. Gives the embedder a chance to provide an already
     * bound connection if it has one. (allowing for warm-up connections: connections that are
     * already bound in advance to speed up child process start-up time). Note that
     * onBeforeConnectionAllocated will not be called if this method returns a connection.
     *
     * @param connectionAllocator the allocator the returned connection should have been allocated
     *     of.
    ...
    /**
     * Called before a connection is allocated. Note that this is only called if the
     * ChildProcessLauncher is created with {@link #createWithConnectionAllocator}.
     *
     ...
     *
     * @param connectionBundle the bundle passed to the {@link ChildProcessService} in the setup
     *     call. Clients can add their own extras to the bundle.
     */
   ...
   *
   * @param setupConnection whether the setup should be performed on the connection once established
  ...
  public boolean start(final boolean setupConnection, final boolean queueIfNoFreeConnection) {
    ...
    try {
      ...
      if (!allocateAndSetupConnection(serviceCallback, setupConnection, queueIfNoFreeConnection)
