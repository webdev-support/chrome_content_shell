...
public class ChildProcessConnection {
  ...
  /**
   * Used to notify the consumer about the process start. These callbacks will be invoked before the
   * ConnectionCallbacks.
   */
  public interface ServiceCallback {
    ...
    /**
     * Called when the child process failed to start. This can happen if the process is already in
     * use by another client. The client will not receive any other callbacks after this one.
     */
    void onChildStartFailed(ChildProcessConnection connection);

    /**
     * Called when the service has been disconnected. whether it was stopped by the client or if it
     * stopped unexpectedly (process crash). This is the last callback from this interface that a
     * client will receive for a specific connection.
     */
  ...
  public interface ZygoteInfoCallback {
    /**
     * Called after the connection has been established, only once per process being connected to.
     *
     * @param connection the connection object to the child process. Must hold the zygote PID that
     *     the child process was spawned from
     * @param relroBundle the bundle potentially containing the information making it possible to
     *     replace the current RELRO address range to memory shared with the zpp zygote. Needs to be
     *     passed to the library loader in order to take effect. Can be done before or after the
     *     LibraryLoader loads the library.
     */
  ...
  public ChildProcessConnection(
    ...
    mServiceBundle.putString(
        ChildProcessConstants.EXTRA_BROWSER_PACKAGE_NAME, BuildInfo.getInstance().packageName);
    ...
    createBindings(
        sAlwaysFallback && mFallbackServiceName != null ? mFallbackServiceName : mServiceName);
  }
  ...
  private void createBindings(ComponentName serviceName) {
    ...
    int defaultFlags =
        Context.BIND_AUTO_CREATE | (mBindAsExternalService ? Context.BIND_EXTERNAL_SERVICE : 0);
    ...
    mWaivedBinding =
        mConnectionFactory.createConnection(
            intent, defaultFlags | Context.BIND_WAIVE_PRIORITY, mConnectionDelegate, mInstanceName);
  }
      ...
      try {
        ...
        childMatches =
            Objects.equals(parent.sourceDir, child.sourceDir)
                && Arrays.equals(parent.sharedLibraryFiles, child.sharedLibraryFiles);
      } catch (RemoteException ex) {
  ...
  /**
   * Called after the connection parameters have been set (in setupConnection()) *and* a connection
   * has been established (as signaled by onServiceConnected()). These two events can happen in any
   * order.
   */
  ...
  private boolean bind(boolean useStrongBinding) {
    ...
    if (success && !usedFallback && canFallback) {
      mLauncherHandler.postDelayed(this::checkBindTimeOut, FALLBACK_TIMEOUT_IN_SECONDS * 1000);
    }
