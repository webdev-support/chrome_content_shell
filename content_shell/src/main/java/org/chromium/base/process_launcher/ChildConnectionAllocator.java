...
public abstract class ChildConnectionAllocator {
  ...
  public static ChildConnectionAllocator createVariableSize(
    ...
    boolean disableZygote =
        Build.VERSION.SDK_INT == 29 && buildInfo.androidBuildFingerprint.startsWith("OnePlus/");

    if (Build.VERSION.SDK_INT == 29 && !disableZygote) {
      UserManager userManager =
          (UserManager) ContextUtils.getApplicationContext().getSystemService(Context.USER_SERVICE);
    ...
    String fallbackServiceClassName = disableZygote ? null : serviceClassName + NON_ZYGOTE_SUFFIX;
  ...
  public void setConnectionFactoryForTesting(ConnectionFactory connectionFactory) {
    ConnectionFactory oldValue = mConnectionFactory;
  ...
  /* package */
  abstract ChildProcessConnection doAllocate(
      Context context,
      Bundle serviceBundle,
      ChildProcessConnection.ServiceCallback serviceCallback);

  /* package */
  abstract void doFree(ChildProcessConnection connection);
