...
public class TransitiveObservableSupplier<P, T> implements ObservableSupplier<T> {
  ...
  private final @NonNull Callback<P> mOnParentSupplierChangeCallback = this::onParentSupplierChange;
  private final @NonNull Callback<T> mOnTargetSupplierChangeCallback = this::onTargetSupplierChange;
  ...
  public TransitiveObservableSupplier(
      ObservableSupplier<P> parentSupplier, Function<P, ObservableSupplier<T>> unwrapFunction) {
   ...
   * to the new target supplier. In practice this is full of null checks. We also have to make sure
   * we keep our delegate supplier's value up to date, which is also what drives client
   * observations.
   */
  private void onParentSupplierChange(@Nullable P parentValue) {
    ...
    if (mCurrentTargetSupplier == null) {
      ...
      onTargetSupplierChange(mCurrentTargetSupplier.addObserver(mOnTargetSupplierChangeCallback));
    }
  }
