...
public class SupplierUtils {
    ...
    void waitForAll(Runnable callback, Supplier... suppliers) {
      ...
      for (Supplier<?> supplier : suppliers) {
        ...
        if (supplier instanceof ObservableSupplier) {
          ...
          assert false : "Unexpected Supplier type that does not already have a value: " + supplier;
        }
      }
  ...
  /**
   * Waits for all suppliers to have assigned values, and when that happens, notifies the specified
   * callback.
   *
   ...
   *
   * <p>Not thread safe. All passed in suppliers must be notified on the same thread this method is
   * called.
   *
