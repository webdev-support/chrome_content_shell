...
public interface LazyOneshotSupplier<T> {
  /**
   * Add a callback that's called when the object owned by this supplier is available. If the object
   * is already available, the callback will be called at the end of the current message loop.
   *
  ...
  /**
   * Returns the value currently held or <code>null</code> when none is held. Use {@link #hasValue}
   * to tell if the value is intentionally null.
   */
  @Nullable
  T get();
   ...
   * when the value already exists or in tests, as otherwise it defeats the purpose of the lazy part
   * of this supplier.
   */
  static <T> LazyOneshotSupplier<T> fromValue(T value) {
    return new LazyOneshotSupplierImpl<T>() {
      @Override
      public void doSet() {
        set(value);
      }
