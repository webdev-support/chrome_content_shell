...
public class BuildInfo {
  ...
  /**
   * By default: same as versionCode. For WebView: versionCode of the embedding app. In the context
   * of the SDK Runtime, this is the versionCode of the app that owns this particular instance of
   * the SDK Runtime.
   */
  ...
  private String[] getAllProperties() {
    ...
    return new String[] {
      ...
      customThemes,
      resourcesVersion,
      String.valueOf(ContextUtils.getApplicationContext().getApplicationInfo().targetSdkVersion),
      isDebugAndroid() ? "1" : "0",
      isTV ? "1" : "0",
  ...
  /**
   * Return the "long" version code of the given PackageInfo. Does the right thing for before/after
   * Android P when this got wider.
   */
  ...
  private BuildInfo() {
    ...
    if (CommandLine.isInitialized()) {
      ...
      providedPackageVersionName = commandLine.getSwitchValue(BaseSwitches.PACKAGE_VERSION_NAME);
      ...
      if (commandLine.hasSwitch(BaseSwitches.PACKAGE_VERSION_CODE)) {
        providedPackageVersionCode =
            Long.parseLong(commandLine.getSwitchValue(BaseSwitches.PACKAGE_VERSION_CODE));
      }
    }
    ...
    if (BuildConfig.R_STRING_PRODUCT_VERSION != 0) {
      try {
        ...
        currentResourcesVersion =
            ContextUtils.getApplicationContext().getString(BuildConfig.R_STRING_PRODUCT_VERSION);
      } catch (Exception e) {
  ...
  /**
   * Check if this is a debuggable build of Android. This is a rough approximation of the hidden API
   * {@code Build.IS_DEBUGGABLE}.
   */
   ...
   *     WebView callers should just inline the logic below to check the target level of the
   *     embedding App when necessary.
   */
  @Deprecated
