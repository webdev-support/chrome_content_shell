...
public interface Callback<T> {
   ...
   *
   * <p>For example, instead of: mView.post(() -> myCallback.onResult(result)); Avoid creating an
   * inner class via: mView.post(myCallback.bind(result));
   */
  ...
  /**
   * JNI Generator does not know how to target static methods on interfaces (which is new in Java 8,
   * and requires desugaring).
   */
