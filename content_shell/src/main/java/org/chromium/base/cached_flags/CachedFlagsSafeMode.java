...
public class CachedFlagsSafeMode {
  ...
  /**
   * Call right before any flag is checked. The first time this is called, check if safe mode should
   * be engaged, and engages it if necessary.
   */
  @AnyThread
  void onFlagChecked() {
    synchronized (mBehavior) {
      ...
      if (shouldEnterSafeMode()) {
        String cachedVersion = getSafeValuePreferences().getString(PREF_SAFE_VALUES_VERSION, "");
        int behavior;
  ...
  /**
   * Call when aborting a path that leads to caching flags. Rolls back the crash streak incremented
   * in {@link #onStartOrResumeCheckpoint} but does not reset it.
   */
  ...
  private boolean shouldEnterSafeMode() {
    if (BuildConfig.IS_FOR_TEST
        && (mSafeModeExperimentForcedForTesting == null || !mSafeModeExperimentForcedForTesting)) {
      return false;
    }
    ...
    if (safeModeRunsLeft > 0) {
      CachedFlagsSharedPreferences.getInstance()
          .writeInt(CachedFlagsSharedPreferences.FLAGS_SAFE_MODE_RUNS_LEFT, safeModeRunsLeft - 1);
