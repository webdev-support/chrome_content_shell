 ...
 * When a scenario stops it takes its metrics and sends them to native to be recorded in UMA. This
 * is executed by JankReportingScheduler on its own thread.
 */
    ...
    public void run() {
      try (TraceEvent e = TraceEvent.scoped("ReportingCUJScenarioData", mScenario.type())) {
        ...
        if (mJankEndScenarioTime == null) {
          ...
          metrics =
              mMetricsStore.stopTrackingScenario(mScenario, mJankEndScenarioTime.endScenarioTimeNs);
        }
        ...
        JankMetricUMARecorder.recordJankMetricsToUMA(metrics, startTime, endTime, mScenario.type());
      }
    }
  }
    ...
    try (TraceEvent e =
      ...
      boolean dataIsReady =
          mJankEndScenarioTime == null
              || (mJankEndScenarioTime != null
                  && mMetricsStore.hasReceivedMetricsPast(mJankEndScenarioTime.endScenarioTimeNs));
