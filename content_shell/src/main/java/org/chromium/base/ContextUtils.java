...
public class ContextUtils {
  ...
  /**
   * Flag for {@link Context#registerReceiver}: The receiver can receive broadcasts from other Apps.
   ...
   *
   * <p>Under normal circumstances there is only one application context in a process, so it's safe
   * to treat this as a global. In WebView it's possible for more than one app using WebView to be
   * running in a single process, but this mechanism is rarely used and this is not the only problem
   * in that scenario, so we don't currently forbid using it as a global.
   *
   * <p>Do not downcast the context returned by this method to Application (or any subclass). It may
   ...
   * preferences. This avoids needless I/O for android N and above. It also makes it clear that the
   * app-wide shared preference is desired, rather than the potentially context-specific one.
   *
  ...
  /**
   * In most cases, {@link Context#getAssets()} can be used directly. Modified resources are used
   * downstream and are set up on application startup, and this method provides access to regular
   * assets before that initialization is complete.
   *
   * <p>This method should ONLY be used for accessing files within the assets folder.
   *
   ...
   * broadcasts, so you should make sure that the broadcasts you register for are in fact protected
   * broadcasts. The Android platform's <a
   ...
   * in the platform's manifest, as they may not be protected on all devices. Different versions or
   * builds of Android may have different sets of protected broadcasts, so add appropriate guards if
   * needed.
   *
  ...
  /**
   * Register a broadcast receiver that may only accept broadcasts coming from the root, system, or
   * this app's own UIDs.
   *
   * <p>You should generally prefer using this over the exported counterpart, {@link
   * #registerExportedBroadcastReceiver(Context, BroadcastReceiver, IntentFilter, String)}, unless
   * you meet a specific requirement specified in that method's documentation.
   *
   * <p>Even though most protected broadcasts come from the system UID, and could thus be received
   * by a non-exported receiver, you should instead use registerProtectedBroadcastReceiver for all
   * protected broadcasts.
   *
   * <p>You should (only) use non-exported receivers when:
   *
   * <p>
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>Note that older versions of Android do not enforce non-exported receivers, so you should
   ...
   * inherit the permissions of the embedding application. Consider using {@link
   * org.chromium.base.IntentUtils#addTrustedIntentExtras} and {@link
   * org.chromium.base.IntentUtils#isTrustedIntentFromSelf} to verify the Intent's sender.
   *
   * <p>Usually, when working with non-exported receivers, you should also make sure that any
   * related Intents that you send are not broadcast to other apps. This can be done using {@link
   * Intent#setPackage} with {@link Context#getPackageName}, and must be done before calling {@link
   * org.chromium.base.IntentUtils#addTrustedIntentExtras}.
   *
   * <p>You can unregister receivers using the normal {@link Context#unregisterReceiver} method.
   */
