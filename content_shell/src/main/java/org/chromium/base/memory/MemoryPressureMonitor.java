...
/**
 * This class monitors memory pressure and reports it to the native side. Even though there can be
 * other callbacks besides MemoryPressureListener (which reports pressure to the native side, and is
 * added implicitly), the class is designed to suite needs of native MemoryPressureListeners.
 *
 * <p>There are two groups of MemoryPressureListeners:
 *
 * <p>1. Stateless, i.e. ones that simply free memory (caches, etc.) in response to memory pressure.
 * These listeners need to be called periodically (to have effect), but not too frequently (to avoid
 * regressing performance too much).
 *
 * <p>2. Stateful, i.e. ones that change their behavior based on the last received memory pressure
 * (in addition to freeing memory). These listeners need to know when the pressure subsides, i.e.
 * they need to be notified about CRITICAL->MODERATE changes.
 *
 * <p>Android notifies about memory pressure through onTrimMemory() / onLowMemory() callbacks from
 * ComponentCallbacks2, but these are unreliable (e.g. called too early, called just once, not
 * called when memory pressure subsides, etc., see https://crbug.com/813909 for more examples).
 *
 * <p>There is also ActivityManager.getMyMemoryState() API which returns current pressure for the
 * calling process. It has its caveats, for example it can't be called from isolated processes
 * (renderers). Plus we don't want to poll getMyMemoryState() unnecessarily, for example there is no
 * reason to poll it when Chrome is in the background.
 *
 * <p>This class implements the following principles:
 *
 * <p>1. Throttle pressure signals sent to callbacks. Callbacks are called at most once during
 * throttling interval. If same pressure is reported several times during the interval, all reports
 * except the first one are ignored.
 *
 * <p>2. Always report changes in pressure. If pressure changes during the interval, the change is
 * not ignored, but delayed until the end of the interval.
 *
 * <p>3. Poll on CRITICAL memory pressure. Once CRITICAL pressure is reported, getMyMemoryState API
 * is used to periodically query pressure until it subsides (becomes non-CRITICAL).
 *
 * <p>Zooming out, the class is used as follows:
 *
 * <p>1. Only the browser process / WebView process poll, and it only polls when it makes sense to
 * do so (when Chrome is in the foreground / there are WebView instances around).
 *
 * <p>2. Services (GPU, renderers) don't poll, instead they get additional pressure signals from the
 * main process.
 *
 * <p>NOTE: This class should only be used on UiThread as defined by ThreadUtils (which is Android
 * main thread for Chrome, but can be some other thread for WebView).
 */
public class MemoryPressureMonitor {
  ...
  /**
   * Notifies the class about change in memory pressure. Note that |pressure| might get throttled or
   * delayed, i.e. calling this method doesn't necessarily call the callbacks. See the class
   * comment.
   */
  ...
  /**
   * Last pressure that was reported to MemoryPressureListener. Returns MemoryPressureLevel.NONE if
  ...
  /**
   * Maps ComponentCallbacks2.TRIM_* value to MemoryPressureLevel. Returns null if |level| couldn't
   * be mapped and should be ignored.
   */
  @VisibleForTesting
