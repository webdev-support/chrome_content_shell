...
public abstract class PathUtils {
  ...
  /**
   * Get the directory paths from sDirPathFetchTask if available, or compute it synchronously on the
   * UI thread otherwise. This should only be called as part of Holder's initialization above to
   * guarantee thread-safety as part of the initialization-on-demand holder idiom.
   */
  ...
  /**
   * Fetch the path of the directory where private data is to be stored by the application. This is
   * meant to be called in an FutureTask in setPrivateDataDirectorySuffix(), but if we need the
   * result before the FutureTask has had a chance to finish, then it's best to cancel the task and
   * run it on the UI thread instead, inside getOrComputeDirectoryPaths().
   *
   ...
   * want to cancel it and run on the UI thread instead. Unfortunately, this means keeping a bit of
   * extra static state - we need to store the suffix and the application context in case we need to
   * try to re-execute later.
   *
   * @param dataBasePath The base path for the data directory. If null, defaults to using Android
   *     Platform specific app data directory.
   * @param cacheBasePath The base path for the cache directory. If null, defaults to using Android
   *     Platform specific app cache directory.
   ...
   * will block public directory access. May return empty string when there are no external storage
   * volumes mounted.
   */
  ...
  /**
   * @return Download directories including the default storage directory on SD card, and a private
   *     directory on external SD card.
   */
  @SuppressWarnings("unused")
  @CalledByNative
