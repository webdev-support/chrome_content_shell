...
public abstract class AsyncTask<Result> {
   ...
   * of a task. AsyncTaskStatus corresponding to this is defined in
   * tools/metrics/histograms/enums.xml. Entries should not be renumbered and numeric values should
   * never be reused.
   */
  ...
  /**
   * Returns the current status of this task, with adjustments made to make UMA more useful. Namely,
   * we are going to return "PENDING" until the asynctask actually starts running. Right now, as
   * soon as you try to schedule the AsyncTask, it gets set to "RUNNING" which doesn't make sense.
   * However, we aren't fixing this globally as this is the well-defined API AsyncTasks have, so we
   * are just fixing this for our UMA reporting.
   *
   * @return The current status.
   */
   ...
   * checked periodically from {@link #doInBackground()} to end the task as soon as possible.
   *
   ...
   * thread after {@link #doInBackground()} returns. Calling this method guarantees that {@link
   * #onPostExecute(Object)} is never invoked. After invoking this method, you should check the
   * value returned by {@link #isCancelled()} periodically from {@link #doInBackground()} to finish
