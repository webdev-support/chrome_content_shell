 ...
 * makes sense when submitting tasks to SingleThreadTaskRunners.
 *
...
public class ChainedTasks {
  ...
  private final Runnable mRunAndPost =
      new Runnable() {
        ...
        public void run() {
          ...
          try (TraceEvent e =
              TraceEvent.scoped("ChainedTask.run: " + pair.second.getClass().getName())) {
            pair.second.run();
          }
  ...
  /**
   * Adds a task to the list of tasks to run. Cannot be called once {@link start()} has been called.
   */
   ...
   *
   * @param coalesceTasks if false, posts the tasks. Otherwise run them in a single task. If called
   *     on the thread matching the TaskTraits, will block and run all tasks synchronously.
   */
