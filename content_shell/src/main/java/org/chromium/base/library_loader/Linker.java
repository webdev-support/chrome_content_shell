...
class Linker {
   ...
   *
   * <p>The states are: - UNINITIALIZED: Initial state. - INITIALIZED: After linker initialization.
   * Required for using the linker.
   *
   * <p>When loading a library, there are two possibilities:
   *
   * <p>- RELRO is not shared.
   *
   * <p>- RELRO is shared: the producer process loads the library, consumers load the native library
   * without waiting, they use the RELRO bundle later when it arrives, or immediately if it arrived
   * before load
   *
   * <p>Once the library has been loaded, in the producer process the state is DONE_PROVIDE_RELRO,
   * and in consumer processes it is DONE.
   *
   * <p>Transitions are: All processes: UNINITIALIZED -> INITIALIZED Producer: INITIALIZED ->
   * DONE_PROVIDE_RELRO Consumer: INITIALIZED -> DONE
   *
   * <p>When RELRO sharing failed for one reason or another, the state transitions remain the same,
   ...
   *
   * <p>The Linker abstracts away from knowing process types and what the role of each process is.
   ...
   * strategy.
   *
   * <p>In many cases finding the library load address is on the critical path, and needs to be
   * transferred to other processes as soon as possible. For this purpose initialization is invoked
   * separately from loading.
   *
   * <p>The caller should provide the |preference| for obtaining the address at which to load the
   * native library. The value is one of these:
   *
   * <p>FIND_RESERVED, RESERVE_HINT, RESERVE_RANDOM.
   *
   * <p>In the vast majority of cases the chosen preference will be fulfilled and the address (with
   ...
   * made with |preference| to the right of the current one in the list above. There is no fallback
   * for RESERVE_RANDOM.
   *
   * <p>FIND_RESERVED: Finds the (named) reserved address range for library loading. The caller
   * needs to make sure that this is only called on platform versions supporting this memory
   * reservation (Android Q+).
   *
   * <p>RESERVE_HINT: Reserves enough of address range for loading a library, starting at the
   ...
   *
   * <p>RESERVE_RANDOM: Finds a free random address range and reserves it.
   *
  ...
  final void ensureInitialized(
    ...
    if (DEBUG) {
      Log.i(
          TAG,
          "ensureInitialized(asRelroProducer=%b, preference=%s, " + "loadAddressHint=0x%x)",
          asRelroProducer,
  ...
  private void chooseAndReserveMemoryRange(
    ...
    switch (preference) {
      case PreferAddress.FIND_RESERVED:
        boolean reservationFound = getLinkerJni().findRegionReservedByWebViewZygote(mLocalLibInfo);
        if (reservationFound) {
  ...
  /**
   * Extracts the native library start address as serialized by {@link
   * #putLoadAddressToBundle(Bundle)} in a Linker instance from another process.
   */
   ...
   *
   * <p>The library must not be the Chromium linker library.
   *
   * @param library The library name to load.
   */
    ...
    synchronized (mLock) {
      try {
        ...
        attemptLoadLibraryLocked(
            library, mRelroProducer ? RelroSharingMode.PRODUCE : RelroSharingMode.CONSUME);
  ...
  void putSharedRelrosToBundle(Bundle bundle) {
    ...
    synchronized (mLock) {
      ...
      if (DEBUG && relros != null) {
        Log.i(
            TAG,
            "putSharedRelrosToBundle() puts mLoadAddress=0x%x, mLoadSize=%d, " + "mRelroFd=%d",
  ...
  private void loadWithoutProducingRelro(String libFilePath) {
    ...
    if (!getLinkerJni().loadLibrary(libFilePath, mLocalLibInfo, /* spawnRelroRegion= */ false)) {
   ...
   *
   * <p>If the library is within a zip file, it must be uncompressed and page aligned in this file.
   *
   * <p>The {@link #atomicReplaceRelroLocked(boolean)} must be implemented to *atomically* replace
   * the RELRO region. Atomicity is required because the library code can be running concurrently on
   * another thread.
   *
   * @param library The name of the library to load.
   * @param relroMode Tells whether to use RELRO sharing and whether to produce or consume the RELRO
   *     region.
   */
  @GuardedBy("mLock")
  ...
  /**
   * Atomically replaces the RELRO with the shared memory region described in the |mRemoteLibInfo|.
   * In order to perform the replacement verifies that the replacement is safe by inspecting
   * |mLocalLibInfo| for equality of the library address range and the contents of the RELRO region.
   *
   * @param relroAvailableImmediately Whether the RELRO bundle arrived before {@link
   *     #loadLibraryImplLocked(String, int)} was called.
   */
  @GuardedBy("mLock")
   ...
   *
   * <p>Initializes as a RELRO producer without knowledge about preferred placement of the RELRO
  ...
  interface Natives {
     ...
     *
     * <p>On failure |libInfo.mLoadAddress| should be set to 0 and the LibraryLoader will fall back
     * to loading using the system linker.
     *
     * @param libInfo holds the output values: |mLoadAddress| and |mLoadSize|. On failure sets the
     ...
     * ms).
     *
     * @param libInfo holds the output values: |mLoadAddress| and |mLoadSize|. On success saves the
     *     start address and the size of the webview memory reservation to them.
     ...
     * @param libInfo holds the information about the loaded library and the associated RELRO region
     *     if the latter was created.
