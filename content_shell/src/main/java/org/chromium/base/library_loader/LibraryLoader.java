...
public class LibraryLoader {
   ...
   * #takeLoadAddressFromBundle(Bundle)} should be called for passing the RELRO information between
   * library loaders.
   *
  ...
  public class MultiProcessMediator {
    ...
    public void ensureInitializedInMainProcess() {
      ...
      if (useChromiumLinker()) {
        ...
        // delayed startup.
        if (DEBUG) {
          Log.i(TAG, "ensureInitializedInMainProcess, producing RELRO FD: %b", attemptProduceRelro);
        }
        ...
        getLinker().ensureInitialized(attemptProduceRelro, preferAddress, /* addressHint= */ 0);
      }
    ...
    /**
     * Serializes the load address for communication, if any was determined during initialization.
     * Must be called after the library has been loaded in this process.
     *
    ...
    public void initInAppZygote() {
      ...
      if (useChromiumLinker() && !mainProcessIntendsToProvideRelroFd()) {
        getLinker()
            .ensureInitialized(/* asRelroProducer= */ true, Linker.PreferAddress.FIND_RESERVED, 0);
      } else {
    ...
    /**
     * Initializes in processes other than "Main". Can be called only once in each non-main process.
     */
    public void initInChildProcess() {
      ...
      if (mainProcessIntendsToProvideRelroFd()) {
        ...
        getLinker()
            .ensureInitialized(
                /* asRelroProducer= */ false, Linker.PreferAddress.RESERVE_HINT, getLoadAddress());
      } else if (isLoadedByZygote()) {
        if (DEBUG) {
          Log.i(
              TAG,
              "initInChildProcess: already loaded by app zygote " + "(mFallbackToSystemLinker=%b)",
        ...
        if (DEBUG) {
          Log.i(
              TAG,
              "initInChildProcess: child process not from app zygote, with address " + "hint: 0x%x",
              getLoadAddress());
        }
        getLinker()
            .ensureInitialized(
                /* asRelroProducer= */ false, Linker.PreferAddress.FIND_RESERVED, getLoadAddress());
      }
    ...
    /**
     * Optionally extracts RELRO and saves it for replacing the RELRO section in this process. Can
     * be invoked before initialization.
     *
    ...
    public void recordLoadThreadTimeHistogram(long threadLoadTimeMs) {
      RecordHistogram.recordTimesHistogram(
          LINKER_HISTOGRAM_PREFIX + creationAsString() + "ThreadLoadTime", threadLoadTimeMs);
    }
  }
   ...
   *
   * <p>Since this function is called extremely early on in startup, locking is not required.
   *
  ...
  public void setLibraryProcessType(@LibraryProcessType int type) {
    ...
    if (mLibraryProcessType != LibraryProcessType.PROCESS_UNINITIALIZED) {
      throw new IllegalStateException(
          String.format(
              "Trying to change the LibraryProcessType from %d to %d", mLibraryProcessType, type));
    }
   ...
   *
   * <p>Must be called before loading the library. Since this function is called extremely early on
   ...
   *
   * <p>On N, O and P Monochrome is selected by Play Store. With Monochrome this code is not used,
  ...
  /**
   * Blocks until the library is fully loaded and initialized. When this method is used (without the
   * {@link MultiProcessMediator}) the current process is treated as the Main process (w.r.t. how it
   * shares RELRO and reports metrics) unless it was initialized before.
   */
  ...
  /**
   * This method blocks until the native library is initialized, and the Main Dex is loaded (MainDex
   * JNI is registered).
   *
   * <p>You should use this if you would like to use isolated parts of the native library that don't
   * depend on content initialization, and only use MainDex classes with JNI.
   *
   * <p>However, you should be careful not to call this too early in startup on the UI thread, or
   * you may significantly increase the time to first draw.
   */
  ...
  /**
   * Calls native library preloader (see {@link #setNativeLibraryPreloader}) with the app context.
   * If there is no preloader set, this function does nothing. Preloader is called only once, so
   * calling it explicitly via this method means that it won't be (implicitly) called during library
   * loading.
   */
  ...
  /**
   * Initializes the native library: must be called on the thread that the native will call its
   * "main" thread. The library must have previously been loaded with one of the loadNow*()
   * variants.
   */
   ...
   * can do considering that the thread pool has to be configured before finch is initialized. Note
   * that since LibraryLoader is in //base, it can't depend on ChromeFeatureList, and has to rely on
   * external code pushing the value.
   *
  ...
  public static boolean isBackgroundThreadPoolEnabled() {
    try (StrictModeContext ignored = StrictModeContext.allowDiskReads()) {
      return ContextUtils.getAppSharedPreferences().getBoolean(BACKGROUND_THREAD_POOL_KEY, false);
    }
  }
  ...
  public static void setLibraryLoaderForTesting(LibraryLoader loader) {
    LibraryLoader oldValue = sInstance;
    sInstance = loader;
   ...
   * libraries are loaded because ubsan reads its configuration from $UBSAN_OPTIONS when the native
   * library is loaded.
   */
  ...
  /**
   * This sets the LibraryLoader internal state to its fully initialized state and should *only* be
   * used by clients like NativeTests which manually load their native libraries without using the
   * LibraryLoader.
   *
