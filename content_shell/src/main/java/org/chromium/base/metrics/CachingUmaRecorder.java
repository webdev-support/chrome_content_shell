...
/* package */ final class CachingUmaRecorder implements UmaRecorder {
  ...
  /**
   * Maximum number of histograms cached at the same time. It is better to drop some samples rather
   * than have a bug cause the cache to grow without limit.
   *
   * <p>Each sample uses 4 bytes, each histogram uses approx. 12 references (at least 4 bytes each).
   ...
   *
   * <p>A read-write lock is used rather than {@code synchronized} blocks to the limit opportunities
   ...
   * reached.
   *
   * <p>Using {@link AtomicInteger} because the value may need to be updated with a read lock held.
   */
  ...
  /**
   * Number of user actions that couldn't be cached, because the number of user actions in cache has
   * reached its limit.
   */
  @GuardedBy("mRwLock")
   ...
   *
   * <p>The read lock must be held while invoking methods on {@code mDelegate}.
   */
  @GuardedBy("mRwLock")
  @Nullable
  ...
  /**
   * Sets the current delegate to {@code recorder}. Forwards and clears all cached metrics if {@code
   * recorder} is not {@code null}.
   *
  ...
  /**
   * Writes user actions from {@code cache} to the delegate. Assumes that a read lock is held by the
   * current thread.
   *
   * @param cache the cache to be flushed.
   * @param droppedUserActionCount number of user actions that were not recorded in {@code cache} to
   *     stay within {@link MAX_USER_ACTION_COUNT}.
   */
  ...
  /**
   * Appends a histogram {@code sample} to a cached {@link Histogram}. Creates the {@code Histogram}
  ...
  public void recordExponentialHistogram(
    ...
    cacheOrRecordHistogramSample(Histogram.Type.EXPONENTIAL, name, sample, min, max, numBuckets);
  }
  ...
  private void swapUserActionCallbacksForTesting(
    ...
    for (int i = 0; i < mUserActionCallbacksForTesting.size(); i++) {
      if (previousRecorder != null) {
        previousRecorder.removeUserActionCallbackForTesting(mUserActionCallbacksForTesting.get(i));
      }
      if (newRecorder != null) {
