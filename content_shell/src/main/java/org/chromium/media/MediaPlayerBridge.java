...
public class MediaPlayerBridge {
  ...
  private class LoadDataUriTask extends AsyncTask<Boolean> {
    ...
    protected void onPostExecute(Boolean result) {
      ...
      if (result) {
        try {
          getLocalPlayer()
              .setDataSource(ContextUtils.getApplicationContext(), Uri.fromFile(mTempFile));
        } catch (IOException e) {
          result = false;
        }
      }
      ...
      MediaPlayerBridgeJni.get()
          .onDidSetDataUriDataSource(mNativeMediaPlayerBridge, MediaPlayerBridge.this, result);
    }
  ...
  /**
   * Returns an AllowedOperations object to show all the operations that are allowed on the media
   * player.
   */
  @CalledByNative
  protected AllowedOperations getAllowedOperations() {
    ...
    try {
      ...
      Method getMetadata =
          player.getClass().getDeclaredMethod("getMetadata", boolean.class, boolean.class);
