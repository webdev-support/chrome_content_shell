...
public class MediaDrmBridge {
  ...
  private static final UUID CLEARKEY_UUID = UUID.fromString("e2719d58-a985-b3c9-781a-b030af78d30e");
   ...
   *     completed.
   *     <p>When false is returned, release() is called within the function, which will notify the
   *     native code with a null MediaCrypto, if needed.
   */
  ...
  /**
   * Check whether the crypto scheme is supported for the given container. If |containerMimeType| is
   * an empty string, we just return whether the crypto scheme is supported.
   *
   ...
   *
   * @return the converted value for FIRST_API_LEVEL if available, 0 otherwise.
   */
  @CalledByNative
  private static int getFirstApiLevel() {
    ...
    try {
      ...
      Log.e(TAG, "Exception while getting system property %s. Using default.", FIRST_API_LEVEL, e);
      firstApiLevel = 0;
    }
  ...
  private static MediaDrmBridge create(
      ...
      long nativeMediaDrmBridge,
      long nativeMediaDrmStorageBridge) {
    Log.i(TAG, "Create MediaDrmBridge with level %s and origin %s", securityLevel, securityOrigin);
    ...
    try {
      ...
      mediaDrmBridge =
          new MediaDrmBridge(
              cryptoScheme, requiresMediaCrypto, nativeMediaDrmBridge, nativeMediaDrmStorageBridge);
  ...
  /**
   * Set the security level that the MediaDrm object uses. This function should be called right
   * after we construct MediaDrmBridge and before we make any other calls.
   *
  ...
  /**
   * Provision the current origin. Normally provisioning will be triggered automatically when
   * MediaCrypto is needed (in the constructor). However, this is available to preprovision an
   * origin separately. MediaDrmBridgeJni.get().onProvisioningComplete() will be called indicating
   * success/failure.
   */
  @CalledByNative
  ...
  private void createSessionFromNative(
      byte[] initData, String mime, int keyType, String[] optionalParamsArray, long promiseId) {
  ...
  private void createSession(
    ...
    if (drmId == null) {
      onPromiseRejected(promiseId, MediaDrmSystemCode.OPEN_SESSION_FAILED, "Open session failed.");
      return;
    }
    ...
    if (request == null) {
      closeSessionNoException(sessionId);
      onPromiseRejected(
          promiseId, MediaDrmSystemCode.GET_KEY_REQUEST_FAILED, "Generate request failed.");
      return;
    }
  ...
  /**
   * Load session back to memory with MediaDrm. Load persistent storage before calling this. It will
   * fail if persistent storage isn't loaded.
   */
  ...
  private void removeSession(byte[] emeId, final long promiseId) {
    ...
    if (sessionId == null) {
      onPromiseRejected(promiseId, MediaDrmSystemCode.INVALID_SESSION_ID, "Session doesn't exist");
      return;
    }
   ...
   * string, which is treated by the native side as "DEFAULT". TODO(jrummell): Revisit this in the
   * future if the security level gets used for more things.
   */
  @CalledByNative
  ...
  private String getVersion() {
    ...
    if (isWidevine()) {
      Log.i(TAG, "oemCryptoBuildInformation: %s", getPropertyString("oemCryptoBuildInformation"));
    }
    return version;
  }
  ...
  private boolean startProvisioningPreQ() {
    ...
    MediaDrmBridgeJni.get()
        .onProvisionRequest(
            mNativeMediaDrmBridge, MediaDrmBridge.this, request.getDefaultUrl(), request.getData());
    return true;
  }
   ...
   *
   * @param isResponseReceived Flag set to true if communication with provision server was
   *     successful.
  ...
  /**
   * Delay session event handler if |mSessionEventDeferrer| exists and matches |sessionId|.
   * Otherwise run the handler immediately.
   */
  ...
  private void onPromiseRejected(
    ...
    if (isNativeMediaDrmBridgeValid()) {
      MediaDrmBridgeJni.get()
          .onPromiseRejected(
              mNativeMediaDrmBridge, MediaDrmBridge.this, promiseId, systemCode, errorMessage);
    }
  }
  ...
  private class EventListener implements MediaDrm.OnEventListener {
    @Override
    public void onEvent(MediaDrm mediaDrm, byte[] drmSessionId, int event, int extra, byte[] data) {
      ...
      if (sessionId == null) {
        ...
        Log.w(TAG, "EventListener: Invalid session %s", SessionId.toHexString(drmSessionId));
        return;
      }
      ...
      switch (event) {
        case MediaDrm.EVENT_KEY_REQUIRED:
          ...
          request =
              getKeyRequest(sessionId, data, sessionInfo.mimeType(), sessionInfo.keyType(), null);
          if (request != null) {
            onSessionMessage(sessionId, request);
          } else {
      ...
      deferEventHandleIfNeeded(
          ...
          new Runnable() {
            ...
            public void run() {
              ...
              SessionInfo sessionInfo = mSessionManager.get(sessionId);
              if (sessionInfo == null) {
                Log.w(TAG, "KeyStatusChange: No info for session %s", sessionId.toHexString());
                return;
              }
              ...
              onSessionKeysChange(
                  sessionId, getKeysInfo(keyInformation).toArray(), hasNewUsableKey, isKeyRelease);
            }
          });
    }
  }
  ...
  private class ExpirationUpdateListener implements MediaDrm.OnExpirationUpdateListener {
    @Override
    public void onExpirationUpdate(MediaDrm md, byte[] drmSessionId, final long expirationTime) {
      final SessionId sessionId = getSessionIdByDrmId(drmSessionId);
  ...
  interface Natives {
    ...
    void onProvisioningComplete(long nativeMediaDrmBridge, MediaDrmBridge caller, boolean success);
