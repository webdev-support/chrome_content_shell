 ...
 * informations. When session is closed, the mapping is also removed.
 *
...
class MediaDrmSessionManager {
  ...
  static class SessionId {
    ...
    /**
     * Convert byte array to hex string for logging. This is modified from BytesToHexString() in
    ...
    /**
     * Create session ID with random generated (UUID.randomUUID) EME session ID. The ID must be
     * unique within the origin of this object's Document over time, including across Documents and
     * browsing sessions. https://w3c.github.io/encrypted-media/#dom-mediakeysession-generaterequest
     *
    ...
    static SessionId createPersistentSessionId(byte[] drmId) {
      byte[] emeId =
          ApiCompatibilityUtils.getBytesUtf8(UUID.randomUUID().toString().replace('-', '0'));
      return new SessionId(emeId, drmId, /* keySetId= */ null);
    }

    /**
     * Create session ID for temporary license session. The DRM session ID is used as EME session
     * ID.
     *
  ...
  static class SessionInfo {
    ...
    private static SessionInfo fromPersistentInfo(PersistentInfo persistentInfo) {
      ...
      SessionId sessionId =
          new SessionId(persistentInfo.emeId(), /* drmId= */ null, persistentInfo.keySetId());
      return new SessionInfo(
          sessionId, persistentInfo.mimeType(), getKeyTypeFromPersistentInfo(persistentInfo));
    }
  ...
  /**
   * Set drm ID. It should only be called for persistent license session without an opened drm
   * session.
   */
  ...
  /** Mark key as released. It should only be called for persistent license session. */
  ...
  /** Remove session and related infomration from memory, but doesn't touch persistent storage. */
