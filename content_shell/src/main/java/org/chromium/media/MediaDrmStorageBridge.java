...
class MediaDrmStorageBridge {
  ...
  void loadInfo(byte[] emeId, Callback<PersistentInfo> cb) {
    if (isNativeMediaDrmStorageValid()) {
      MediaDrmStorageBridgeJni.get()
          .onLoadInfo(mNativeMediaDrmStorageBridge, MediaDrmStorageBridge.this, emeId, cb);
    } else {
      cb.onResult(null);
    }
  }
  ...
  void clearInfo(byte[] emeId, Callback<Boolean> cb) {
    if (isNativeMediaDrmStorageValid()) {
      MediaDrmStorageBridgeJni.get()
          .onClearInfo(mNativeMediaDrmStorageBridge, MediaDrmStorageBridge.this, emeId, cb);
    } else {
      cb.onResult(true);
    }
  }
  ...
  interface Natives {
    void onProvisioned(
        long nativeMediaDrmStorageBridge, MediaDrmStorageBridge caller, Callback<Boolean> cb);
