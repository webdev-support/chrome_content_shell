...
public class VideoCaptureCamera extends VideoCapture
  ...
  static {
    ...
    COLOR_TEMPERATURES_MAP.append(4600, android.hardware.Camera.Parameters.WHITE_BALANCE_TWILIGHT);
    COLOR_TEMPERATURES_MAP.append(5500, android.hardware.Camera.Parameters.WHITE_BALANCE_DAYLIGHT);
    COLOR_TEMPERATURES_MAP.append(
  ...
  public boolean allocate(int width, int height, int frameRate, boolean enableFaceDetection) {
    ...
    Log.d(
        TAG,
        "allocate: Rotation dev=%d, cam=%d, facing back? %s",
        getDeviceRotation(),
  ...
  public void getPhotoCapabilitiesAsync(long callbackId) {
    ...
    // Ignore them.
    builder
        .setInt(PhotoCapabilityInt.MIN_ISO, 0)
    ...
    builder
        .setDouble(PhotoCapabilityDouble.MIN_ZOOM, minZoom)
    ...
    builder.setMeteringModeArray(MeteringModeType.FOCUS, integerArrayListToArray(jniFocusModes));
    ...
    if (focusMode.equals(android.hardware.Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO)
        || focusMode.equals(android.hardware.Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE)
    ...
    builder
        .setDouble(PhotoCapabilityDouble.STEP_EXPOSURE_COMPENSATION, step)
        .setDouble(
    ...
    if (flashModes != null) {
      builder
          .setBool(
              PhotoCapabilityBool.SUPPORTS_TORCH,
              flashModes.contains(android.hardware.Camera.Parameters.FLASH_MODE_TORCH))
          .setBool(
              PhotoCapabilityBool.TORCH,
              android.hardware.Camera.Parameters.FLASH_MODE_TORCH.equals(parameters.getFlashMode()))
          .setBool(
  ...
  public void setPhotoOptions(
    ...
    if (pointsOfInterestSupported && pointsOfInterest2D.length > 0) {
      ...
      mAreaOfInterest =
          new android.hardware.Camera.Area(
              new Rect(
                  Math.max(-1000, centerX - regionWidth / 2),
    ...
    if (hasExposureCompensation) {
      final int unnormalizedExposureCompensation =
          (int) Math.round(exposureCompensation / parameters.getExposureCompensationStep());
    ...
    if (colorTemperature > 0.0) {
      final String closestSetting =
          getClosestWhiteBalance((int) colorTemperature, parameters.getSupportedWhiteBalance());
        ...
        switch (fillLightMode) {
          ...
          case AndroidFillLightMode.AUTO:
            parameters.setFlashMode(
                hasRedEyeReduction && redEyeReduction
  ...
  public void takePhotoAsync(final long callbackId) {
    ...
    if (mPhotoWidth > 0 || mPhotoHeight > 0) {
      ...
      for (android.hardware.Camera.Size size : supportedSizes) {
        final int diff =
            ((mPhotoWidth > 0) ? Math.abs(size.width - mPhotoWidth) : 0)
      ...
      if (minDiff != Integer.MAX_VALUE) {
        Log.d(
            TAG,
            "requested resolution: (%dx%d); matched (%dx%d)",
            mPhotoWidth,
            mPhotoHeight,
            closestSize.width,
            closestSize.height);
  ...
  public void onPreviewFrame(byte[] data, android.hardware.Camera camera) {
    ...
    try {
      ...
      if (data != null && data.length == mExpectedFrameSize) {
        onFrameAvailable(VideoCaptureCamera.this, data, mExpectedFrameSize, getCameraRotation());
      } else {
        onFrameDropped(
            VideoCaptureCamera.this,
