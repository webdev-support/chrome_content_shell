 ...
 *
 * <p>This object is split in two due to the need to ensure that the Mojo watcher does not keep the
 * WebView alive through strong references. It is expected that the WebView be destroyed when it is
 * collected by the runtime.
 *
 * <p>To achieve this, all fields which might transitively point to the WebView are stored in a
 ...
 *
 * <p>{@link RemoteObjectImpl} similarly holds its target weakly; it is held alive via the map held
 * in Internals.
 */
class RemoteObjectHostImpl implements RemoteObjectHost {
  /**
   * Auditor passed on to {@link RemoteObjectImpl}. Should not hold any strong references that may
   * lead to the contents.
   */
  private final RemoteObjectImpl.Auditor mAuditor;

  /**
   * The registry which owns the underlying target objects, if still alive. See the class comment.
   */
  ...
  RemoteObjectHostImpl(
      RemoteObjectImpl.Auditor auditor, RemoteObjectRegistry registry, boolean allowInspection) {
    mAuditor = auditor;
