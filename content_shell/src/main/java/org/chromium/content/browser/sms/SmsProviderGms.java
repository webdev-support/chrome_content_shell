...
/**
 * Simple proxy that provides C++ code with a pathway to the GMS OTP code retrieving APIs. It can
 * operate in three different backend modes which is determined on construction: 1. User Consent:
 * use the user consent API (older method with sub-optimal UX) 2. Verification: use the browser code
 * API (newer method) 3. Auto: prefers to use the verification method but if it is not available
...
public class SmsProviderGms {
  ...
  public SmsProviderGms(
      long smsProviderGmsAndroid, @GmsBackend int backend, boolean isVerificationBackendAvailable) {
  ...
  public void setUserConsentReceiverForTesting(SmsUserConsentReceiver userConsentReceiver) {
    SmsUserConsentReceiver oldValue = mUserConsentReceiver;
  ...
  public void setVerificationReceiverForTesting(SmsVerificationReceiver verificationReceiver) {
    SmsVerificationReceiver oldValue = mVerificationReceiver;
  ...
  public void listen(WindowAndroid window, boolean isLocalRequest) {
    ...
    boolean shouldUseVerificationReceiver =
        mVerificationReceiver != null && (!isLocalRequest || mBackend != GmsBackend.USER_CONSENT);
  ...
  /**
   * Destroys the user consent receiver if the verification receiver succeeded with a local request.
   *
  ...
  @CalledByNative
  private void setClientAndWindow(Wrappers.SmsRetrieverClientWrapper client, WindowAndroid window) {
    assert mClient == null;
    assert mWindow == null;
    mClient = client;
    mWindow = window;
