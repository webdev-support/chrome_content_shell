...
public final class ChildProcessLauncherHelperImpl {
      ...
      new ChildProcessLauncher.Delegate() {
        ...
        public void onBeforeConnectionSetup(Bundle connectionBundle) {
          ...
          if (sZygoteBundle != null) {
            ...
            LibraryLoader.getInstance().getMediator().putSharedRelrosToBundle(connectionBundle);
          }
        }
        ...
        public void onConnectionEstablished(ChildProcessConnection connection) {
          ...
          if (pid > 0) {
            ...
            if (mReducePriorityOnBackground && !ApplicationStatus.hasVisibleActivities()) {
          ...
          if (mNativeChildProcessLauncherHelper != 0) {
            ChildProcessLauncherHelperImplJni.get()
                .onChildProcessStarted(mNativeChildProcessLauncherHelper, connection.getPid());
          }
        ...
        public void onConnectionLost(ChildProcessConnection connection) {
          ...
          ChildProcessLauncherHelperImpl result = sLauncherByPid.remove(connection.getPid());
   ...
   *     process did not inherit from the app zygote or the app zygote did not produce a usable
   *     RELRO region.
   */
  ...
  private static ChildProcessLauncherHelperImpl createAndStart(
    ...
    if (!ContentSwitches.SWITCH_RENDERER_PROCESS.equals(processType)) {
      if (ContentSwitches.SWITCH_GPU_PROCESS.equals(processType)) {
        ...
        reducePriorityOnBackground =
            ContentFeatureMap.isEnabled(ContentFeatures.REDUCE_GPU_PRIORITY_ON_BACKGROUND);
      } else {
    ...
    IBinder binderCallback =
        ContentSwitches.SWITCH_GPU_PROCESS.equals(processType) ? new GpuProcessCallback() : null;
  ...
  private static void warmUpOnLauncherThread(Context context, boolean sandboxed) {
    ...
    if (sandboxed) {
      ...
      sSparePrivilegedConntection = new SpareChildConnection(context, allocator, serviceBundle);
    }
  }
  ...
  public static void setSandboxServicesSettingsForTesting(
      ChildConnectionAllocator.ConnectionFactory factory, int serviceCount, String serviceName) {
  ...
  static ChildConnectionAllocator getConnectionAllocator(Context context, boolean sandboxed) {
    ...
    if (sSandboxedChildConnectionAllocator == null) {
      final String packageName = ChildProcessCreationParamsImpl.getPackageNameForSandboxedService();
      Log.d(
          TAG,
          "Create a new ChildConnectionAllocator with package name = %s," + " sandboxed = true",
          packageName);
      ...
      if (sSandboxedServiceFactoryForTesting != null) {
        connectionAllocator.setConnectionFactoryForTesting(sSandboxedServiceFactoryForTesting);
      }
      ...
      if (ChildProcessConnection.supportVariableConnections()) {
        ...
        sSandboxedChildConnectionRanking =
            new ChildProcessRanking(sSandboxedChildConnectionAllocator.getNumberOfServices());
      }
    }
  ...
  /**
   * @return The type of process as specified in the command line at {@link
  ...
  private void getTerminationInfoAndStop(long terminationInfoPtr) {
    ...
    if (exceptionString != null && !mReportedException) {
      ...
      PostTask.postTask(
          TaskTraits.UI_BEST_EFFORT, () -> JavaExceptionReporter.reportStackTrace(exceptionString));
    }
    ChildProcessLauncherHelperImplJni.get()
        .setTerminationInfo(
            terminationInfoPtr,
  ...
  private void setPriority(
    ...
    if (mRanking != null) {
      mRanking.updateConnection(connection, visible, frameDepth, intersectsViewport, importance);
      if (mBindingManager != null) mBindingManager.rankingChanged();
    }
   ...
   *
   * @param callback The callback to notify with the process information. {@code callback} will run
   *     on the same thread this method is called on. That thread must support a {@link
   *     android.os.Looper}.
   */
  public static void getProcessIdsByType(Callback<Map<String, List<Integer>>> callback) {
    ...
    LauncherThread.post(
        () -> {
          ...
          for (Map.Entry<Integer, ChildProcessLauncherHelperImpl> entry :
              sLauncherByPid.entrySet()) {
