...
/**
 * An implementation of {@link InputConnection} to communicate with external input method apps. Note
 * that it is running on IME thread (except for constructor and calls from ImeAdapter) such that it
 * does not block UI thread and returns text values immediately after any change to them. Note that
 * extending {@link BaseInputConnection} is a workaround for some OEM's email client which tries to
 * downcast the {@link InputConnection} from {@link View#onCreateInputConnection} into {@link
 * BaseInputConnection}. We are implementing every function of {@link InputConnection}, so 'extends'
 * here should have no functional effect at all. See crbug.com/616334 for more details.
 */
class ThreadedInputConnection extends BaseInputConnection implements ChromiumBaseInputConnection {
  ...
  private void updateComposingTextOnUiThread(
    ...
    int accentToSend = isPendingAccent ? (mPendingAccent | KeyCharacterMap.COMBINING_ACCENT) : 0;
  ...
  public boolean commitCorrection(CorrectionInfo correctionInfo) {
    if (DEBUG_LOGS) {
      Log.i(TAG, "commitCorrection [%s]", ImeUtils.getCorrectionInfoDebugString(correctionInfo));
    }
    return false;
  }
