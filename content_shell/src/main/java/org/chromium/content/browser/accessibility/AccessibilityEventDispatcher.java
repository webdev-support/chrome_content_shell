 ...
 * throttle and queue AccessibilityEvents that are sent in quick succession. This ensures we do not
 * overload the system and create lag by sending superfluous events.
 */
public class AccessibilityEventDispatcher {
  ...
  /**
   * Callback interface to link {@link WebContentsAccessibilityImpl} with an instance of the {@link
   * AccessibilityEventDispatcher} so we can separate out queueing/throttling logic into a
  ...
  interface Client {
    ...
    /**
     * Build an AccessibilityEvent for the given id and type. Requires a connection through the JNI
     * to the native code to populate the fields of the event. If successfully built, then send the
     * event and return true, otherwise return false.
     *
   ...
   * the appropriate amount of time we should delay, if at all, before building/sending this event.
   * When ready, this will handle the delayed construction and dispatching of this event.
   *
   * @param virtualViewId This virtualViewId for the view trying to send an event.
   * @param eventType The AccessibilityEvent type.
   */
  public void enqueueEvent(int virtualViewId, int eventType) {
    ...
    if (mEventLastFiredTimes.get(uuid) == null
      ...
      mClient.postRunnable(
          myRunnable, (mEventLastFiredTimes.get(uuid) + mEventThrottleDelays.get(eventType)) - now);
