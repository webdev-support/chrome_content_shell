...
public class AccessibilityNodeInfoBuilder {
  ...
  @CalledByNative
  private void addAccessibilityNodeInfoChildren(AccessibilityNodeInfoCompat node, int[] childIds) {
  ...
  private void addAccessibilityNodeInfoActions(
    ...
    if (ContentFeatureMap.isEnabled(ContentFeatureList.ACCESSIBILITY_INCLUDE_LONG_CLICK_ACTION)) {
  ...
  @CalledByNative
  protected void setAccessibilityNodeInfoPaneTitle(AccessibilityNodeInfoCompat node, String title) {
  ...
  private CharSequence computeText(
    ...
    if (suggestionStarts != null && suggestionStarts.length > 0) {
      ...
      for (int i = 0; i < suggestionStarts.length; i++) {
        ...
        if (start < 0 || start > spannableLen || end < 0 || end > spannableLen || start > end) {
          continue;
        }
  ...
  protected void setAccessibilityNodeInfoCollectionItemInfo_exp(
    ...
    setAccessibilityNodeInfoCollectionItemInfo(
        mDelegate.getInfo(virtualViewId), rowIndex, rowSpan, columnIndex, columnSpan, heading);
  }
  ...
  protected void setAccessibilityNodeInfoSelectionAttrs_exp(
    ...
    setAccessibilityNodeInfoSelectionAttrs(mDelegate.getInfo(virtualViewId), startIndex, endIndex);
  }
