...
/**
 * This class is the Java counterpart to the C++ TtsPlatformImplAndroid class. It implements the
 * Android-native text-to-speech code to support the web speech synthesis API.
 *
 * <p>Threading model note: all calls from C++ must happen on the UI thread. Callbacks from Android
 * may happen on a different thread, so we always use PostTask.runOrPostTask(TaskTraits.UI_DEFAULT,
 * ...) when calling back to C++.
 */
...
class TtsPlatformImpl {
  ...
  private static class TtsEngine {
          ...
          new TextToSpeech(
              ...
              status -> {
                if (status == TextToSpeech.SUCCESS) {
                  PostTask.runOrPostTask(TaskTraits.UI_DEFAULT, () -> initializeDefault());
                }
              });
    }
     ...
     * TtsPlatformImplJni.get().voicesChanged directly.
     */
  ...
  private TtsEngine getOrCreateTtsEngine(String engineId) {
    if (!mDefaultTtsEngine.isInitialized()
        ...
        || TextUtils.equals(engineId, mDefaultTtsEngine.getTextToSpeech().getDefaultEngine())
  ...
  /**
   * Create a TtsPlatformImpl object, which is owned by TtsPlatformImplAndroid on the C++ side.
   *
  ...
  /**
   * Called when our C++ counterpoint is deleted. Clear the handle to our native C++ object,
  ...
  /**
   * @return true if our TextToSpeech object is initialized and we've finished scanning the list of
   *     voices.
   */
  @CalledByNative
  private boolean isInitialized() {
   ...
   * @param engineId The ID of the underlying TTS engine to use for this utterance. If not specified
   *     or we are unable to create the engine, we use the default engine.
  ...
  private boolean speak(
    ...
    if (!ttsEngine.isInitialized()) {
      ...
      PendingUtterance pendingUtterance =
          new PendingUtterance(this, utteranceId, text, lang, engineId, rate, pitch, volume);
        ...
        () -> {
          if (mNativeTtsPlatformImplAndroid != 0) {
            TtsPlatformImplJni.get()
                .onEndEvent(mNativeTtsPlatformImplAndroid, Integer.parseInt(utteranceId));
          }
        });
  }
  ...
  private void sendErrorEventOnUiThread(final String utteranceId) {
    PostTask.runOrPostTask(
        ...
        () -> {
          if (mNativeTtsPlatformImplAndroid != 0) {
            TtsPlatformImplJni.get()
                .onErrorEvent(mNativeTtsPlatformImplAndroid, Integer.parseInt(utteranceId));
          }
        });
  }
  ...
  private void sendStartEventOnUiThread(final String utteranceId) {
    PostTask.runOrPostTask(
        ...
        () -> {
          if (mNativeTtsPlatformImplAndroid != 0) {
            TtsPlatformImplJni.get()
                .onStartEvent(mNativeTtsPlatformImplAndroid, Integer.parseInt(utteranceId));
          }
        });
  }
