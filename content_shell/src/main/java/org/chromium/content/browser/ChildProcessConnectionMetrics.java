 ...
 * to the {@link ChildBindingState} that would be applied based on the {@link
 * ChildProcessImportance} in {@link ChildProcessLauncherHelperImpl#setPriority} if {@link
 * BindingManager} was not running.
 *
 * <p>This class enforces that it is only used on the launcher thread other than during init.
 */
public class ChildProcessConnectionMetrics {
    ...
    PostTask.postTask(
        ...
        () -> {
          ...
          mApplicationInForegroundOnUiThread =
              ApplicationStatus.getStateForApplication() == ApplicationState.HAS_RUNNING_ACTIVITIES
  ...
  void emitMetrics() {
    ...
    RecordHistogram.recordCount100Histogram(
        "Android.ChildProcessBinding.NotPerceptibleConnections", notPerceptibleBindingCount);
    RecordHistogram.recordCount100Histogram(
        "Android.ChildProcessBinding.WaivedConnections", waivedBindingCount);

    // Metrics if BindingManager wasn't running.
    RecordHistogram.recordCount100Histogram(
        "Android.ChildProcessBinding.ContentVisibleConnections", contentVisibleBindingCount);
    RecordHistogram.recordCount100Histogram(
