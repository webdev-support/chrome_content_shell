...
/**
 * Cached copy of all positions and scales (CSS-to-DIP-to-physical pixels) reported from the
 * renderer. Provides wrappers and a utility class to help with coordinate transforms on the client
 * side. Provides the internally-visible set of update methods.
 *
 * <p>Unless stated otherwise, all coordinates are in CSS (document) coordinate space.
 */
public class RenderCoordinatesImpl implements RenderCoordinates {
  ...
  /**
   * @return whether the first frame info was passed in and cached. Rendered content area dimension,
   *     page scale factor, etc. is available if true.
   */
