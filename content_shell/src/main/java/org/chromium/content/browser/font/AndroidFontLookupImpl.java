...
public class AndroidFontLookupImpl implements AndroidFontLookup {
  ...
  static final String FETCH_ALL_FONT_FILES_HISTOGRAM = "Android.FontLookup.FetchAllFontFiles.Time";
  ...
  /**
   * Collection of fonts (by ICU case folded full font name) that may be available locally from GMS
   * Core. This collection of Android Downloadable fonts should initially match the fonts listed in
   * {@link FontPreloader}. If/when fonts are determined to be unavailable on-device they will be
   * removed from this set.
   */
  private final Set<String> mExpectedFonts;

  /**
   * Map from ICU case folded full font names to file descriptor for that font. This cache allows us
   * to skip calling GMS Core for fonts that have already been fetched. This map just caches the
   * file descriptors, so the OS shouldn't need to keep the file in memory if we are no longer using
   * it.
   */
   ...
   * "preloaded_fonts" AndroidManifest directive, and have not previously failed a programmatic font
   * fetch request.
   *
   * <p>TODO(crbug.com/1111148): Ensure the font preload by manifest XML is also done for WebView.
   *
   * @param callback The callback to be called with the list of fonts expected (but not guaranteed)
   *     to be available. The list is sorted in ascending order.
   */
  @Override
  ...
  /**
   * Fetches the font file from GMS Core and removes from the expected fonts array if not available.
   *
  ...
  private ParcelFileDescriptor tryFetchFont(String fontUniqueName) {
    ...
    try {
      long startTimeMs = SystemClock.elapsedRealtime();
      FontFamilyResult fontFamilyResult = mFontsContract.fetchFonts(mAppContext, null, request);
      RecordHistogram.recordTimesHistogram(
      ...
      if (fontFamilyResult.getStatusCode() != FontFamilyResult.STATUS_OK) {
        Log.d(TAG, "Font fetch failed with status code: %d", fontFamilyResult.getStatusCode());
        return null;
      }

      FontInfo[] fontInfos = fontFamilyResult.getFonts();
      if (fontInfos.length != 1) {
        Log.d(TAG, "Font fetch did not return a unique result: length = %d", fontInfos.length);
        return null;
      }
  ...
  /**
   * Creates the map from ICU case folded full font name to GMS Core font provider query format, for
   * a selected subset of Android Downloadable fonts.
   *
   * <p>When adding additional fonts to this map: 1. Add the font to the array in {@link
   * FontPreloader} to prefetch new fonts programmatically async during startup. 2. Keys should be
   * ICU case folded full font name. This can be done manually with icu_fold_case_util.cc, or in
   * Java by importing the ICU4J third_party library. (The CaseMap.Fold Java API is only available
   * in Android API 29+.)
   *
  ...
  /**
   * Construct a GMS Core Downloadable fonts query for a font with exact match parameters. (More
   * info: https://developers.google.com/fonts/docs/android#query_format)
   *
  ...
  public static class Factory implements InterfaceFactory<AndroidFontLookup> {
    /**
     * It's safe to store this as a global because there's usually only one application context per
     * process, see {@link ContextUtils#getApplicationContext()} for more info.
     */
