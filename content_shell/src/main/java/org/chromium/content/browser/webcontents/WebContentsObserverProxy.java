...
/**
 * Serves as a compound observer proxy for dispatching WebContentsObserver callbacks, avoiding
...
class WebContentsObserverProxy extends WebContentsObserver {
   ...
   * WebContentsObserver instance will be created, which will observe the native counterpart to the
   * provided WebContents.
   *
  ...
  public void didToggleFullscreenModeForTab(boolean enteredFullscreen, boolean willCauseResize) {
    ...
    Iterator<WebContentsObserver> observersIterator = mObservers.iterator();
    for (; observersIterator.hasNext(); ) {
      observersIterator.next().didToggleFullscreenModeForTab(enteredFullscreen, willCauseResize);
    }
    finishObserverCall();
  }
