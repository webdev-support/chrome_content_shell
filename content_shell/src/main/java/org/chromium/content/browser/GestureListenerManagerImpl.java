...
/**
 * Implementation of the interface {@link GestureListenerManager}. Manages the {@link
 * GestureStateListener} instances, and invokes them upon notification of various events.
...
public class GestureListenerManagerImpl
  ...
  /**
   * Whether a user scroll sequence is active, used to hide text selection handles. Note that a
   * scroll sequence will *always* bound a pinch sequence, so this will also be true for the
   * duration of a pinch gesture. A fling is also always bounded by a gesture scroll sequence so
   * this is also true for the duration of the fling.
   */
   ...
   * @return {@link GestureListenerManager} object used for the give WebContents. Creates one if not
   *     present.
   */
  public static GestureListenerManagerImpl fromWebContents(WebContents webContents) {
    if (sInstanceForTesting != null) return sInstanceForTesting;
    return ((WebContentsImpl) webContents)
        .getOrSetUserData(GestureListenerManagerImpl.class, UserDataFactoryLazyHolder.INSTANCE);
  }
  ...
  public GestureListenerManagerImpl(WebContents webContents) {
    ...
    mNativeGestureListenerManager =
        GestureListenerManagerImplJni.get().init(GestureListenerManagerImpl.this, mWebContents);
  }

  public void resetGestureDetection() {
    if (mNativeGestureListenerManager != 0) {
      GestureListenerManagerImplJni.get()
          .resetGestureDetection(mNativeGestureListenerManager, GestureListenerManagerImpl.this);
    }
  }
  ...
  public void updateMultiTouchZoomSupport(boolean supportsMultiTouchZoom) {
    ...
    GestureListenerManagerImplJni.get()
        .setMultiTouchZoomSupportEnabled(
            mNativeGestureListenerManager, GestureListenerManagerImpl.this, supportsMultiTouchZoom);
  }
  ...
  void onFlingStart(boolean isDirectionUp) {
    ...
    for (mIterator.rewind(); mIterator.hasNext(); ) {
      mIterator
          .next()
          .onFlingStartGesture(verticalScrollOffset(), verticalScrollExtent(), isDirectionUp);
    }
  }
  ...
  private void updateScrollInfo(
    ...
    contentHeight =
        Math.max(contentHeight, containerView.getHeight() / (deviceScale * pageScaleFactor));
    ...
    final boolean scrollChanged =
        pageScaleChanged || scrollOffsetX != rc.getScrollX() || scrollOffsetY != rc.getScrollY();
  ...
  private void onRootScrollOffsetChanged(float scrollOffsetX, float scrollOffsetY) {
    onRootScrollOffsetChangedImpl(
        mWebContents.getRenderCoordinates().getPageScaleFactor(), scrollOffsetX, scrollOffsetY);
  }
  ...
  /**
   * Reset scroll and fling accounting, notifying listeners as appropriate. This is useful as a
   * failsafe when the input stream may have been interruped.
   */
  ...
  interface Natives {
    ...
    void setRootScrollOffsetUpdateFrequency(
        long nativeGestureListenerManager, @RootScrollOffsetUpdateFrequency.EnumType int frequency);
  }
}
