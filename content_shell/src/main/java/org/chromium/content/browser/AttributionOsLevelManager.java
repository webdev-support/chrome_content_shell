...
public class AttributionOsLevelManager {
  ...
  private static void recordOperationResult(@OperationType int type, @OperationResult int result) {
    String suffix = "";
    switch (type) {
  ...
  private void onRegistrationCompleted(
    ...
    if (mNativePtr != 0) {
      AttributionOsLevelManagerJni.get()
          .onRegistrationCompleted(mNativePtr, requestId, result == OperationResult.SUCCESS);
    }
  }
  ...
  private void registerWebAttributionSource(
    ...
    if (!supportsAttribution()) {
      onRegistrationCompleted(
          requestId, OperationType.REGISTER_WEB_SOURCE, OperationResult.ERROR_VERSION_UNSUPPORTED);
      return;
    }
  ...
  private void registerAttributionSource(
    ...
    if (!supportsAttribution()) {
      onRegistrationCompleted(
          requestId, OperationType.REGISTER_SOURCE, OperationResult.ERROR_VERSION_UNSUPPORTED);
      return;
    }
    MeasurementManagerFutures mm = getManager();
    if (mm == null) {
  ...
  private void registerWebAttributionTrigger(
    ...
    if (!supportsAttribution()) {
      onRegistrationCompleted(
          requestId, OperationType.REGISTER_WEB_TRIGGER, OperationResult.ERROR_VERSION_UNSUPPORTED);
      return;
    }
    ...
    ListenableFuture<?> future =
        mm.registerWebTriggerAsync(
            new WebTriggerRegistrationRequest(triggers, Uri.parse(topLevelOrigin.getSpec())));
  ...
  private void registerAttributionTrigger(int requestId, GURL registrationUrl) {
    if (!supportsAttribution()) {
      onRegistrationCompleted(
          requestId, OperationType.REGISTER_TRIGGER, OperationResult.ERROR_VERSION_UNSUPPORTED);
      return;
    }
  ...
  private void deleteRegistrations(
    ...
    MeasurementManagerFutures mm = getManager();
    if (mm == null) {
      recordOperationResult(OperationType.DELETE_REGISTRATIONS, OperationResult.ERROR_INTERNAL);
      onDataDeletionCompleted(requestId);
      return;
    }
    ...
    if (origins.length == 0 && domains.length == 0) {
      switch (matchBehavior) {
        case DeletionRequest.MATCH_BEHAVIOR_DELETE:
          recordOperationResult(OperationType.DELETE_REGISTRATIONS, OperationResult.SUCCESS);
          onDataDeletionCompleted(requestId);
          return;
        case DeletionRequest.MATCH_BEHAVIOR_PRESERVE:
          matchBehaviors =
              ImmutableList.of(
                  DeletionRequest.MATCH_BEHAVIOR_DELETE, DeletionRequest.MATCH_BEHAVIOR_PRESERVE);
          break;
        default:
          Log.e(TAG, "Received invalid match behavior: ", matchBehavior);
          recordOperationResult(OperationType.DELETE_REGISTRATIONS, OperationResult.ERROR_UNKNOWN);
          onDataDeletionCompleted(requestId);
          return;
      }
    } else {
    ...
    FutureCallback<Object> callback =
        new FutureCallback<Object>() {
          ...
          public void onFailure(Throwable thrown) {
            ...
            recordOperationResult(
                OperationType.DELETE_REGISTRATIONS, convertToOperationResult(thrown));
            onCall();
          }
        };
    ...
    for (int currMatchBehavior : matchBehaviors) {
      ...
      Futures.addCallback(future, callback, ContextUtils.getApplicationContext().getMainExecutor());
    }
  }
    ...
    Futures.addCallback(
        ...
        new FutureCallback<Integer>() {
          ...
          public void onFailure(Throwable thrown) {
            ...
            onMeasurementStateReturned(/* status= */ 0, convertToOperationResult(thrown));
          }
        },
        ContextUtils.getApplicationContext().getMainExecutor());
  }
