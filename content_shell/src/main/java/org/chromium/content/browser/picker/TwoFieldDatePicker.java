...
public abstract class TwoFieldDatePicker extends FrameLayout {
  ...
  /**
   * Reorder the date picker spinners to match the order suggested by the locale. Assumes that the
   * order of month and year in the locale is also the right order for the spinner columns.
   */
   ...
   * @param onMonthOrWeekChangedListener How user is notified date is changed by user, can be null.
   */
  public void init(
      int year, int positionInYear, OnMonthOrWeekChangedListener onMonthOrWeekChangedListener) {
    setCurrentDate(year, positionInYear);
    updateSpinners();
  ...
  /** Subclasses know the semantics of @value, and need to return a Calendar corresponding to it. */
