...
class BindingManager implements ComponentCallbacks2 {
  ...
  /**
   * Called when the embedding application is sent to background. The embedder needs to ensure that:
   * - every onBroughtToForeground() is followed by onSentToBackground() - pairs of consecutive
   * onBroughtToForeground() / onSentToBackground() calls do not overlap
   */
  void onSentToBackground() {
    assert LauncherThread.runningOnLauncherThread();

    RecordHistogram.recordCount1000Histogram(
        "Android.BindingManger.ConnectionsDroppedDueToMaxSize", mConnectionsDroppedDueToMaxSize);
