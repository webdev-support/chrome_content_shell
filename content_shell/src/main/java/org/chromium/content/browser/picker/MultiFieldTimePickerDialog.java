...
/**
 * A time picker dialog with upto 5 number pickers left to right: hour, minute, second, milli,
 * AM/PM.
 *
 * <p>If is24hourFormat is true then AM/PM picker is not displayed and hour range is 0..23.
 * Otherwise hour range is 1..12. The milli picker is not displayed if step >= SECOND_IN_MILLIS The
 * second picker is not displayed if step >= MINUTE_IN_MILLIS.
 */
public class MultiFieldTimePickerDialog extends AlertDialog implements OnClickListener {
  ...
  /** Clear focus before retrieving so that values inserted with keyboard are taken into account. */
