 ...
 *
 * <p>For LiveConnect references, an archived version is available at:
...
class RemoteObjectImpl implements RemoteObject {
   ...
   *
   * <p>Separated from this class proper to allow for unit testing in content_junit_tests, where the
   * Android framework is not fully initialized.
   *
   * <p>Implementations should take care not to hold a strong reference to anything that might keep
  ...
  /**
   * Provides numeric identifier for Java objects to be exposed. These identifiers must not collide.
   */
   ...
   *
   * <p>The target object cannot be referred to strongly, because it may contain references which
   * form an uncollectable cycle.
   */
  private final WeakReference<Object> mTarget;

  /**
   * Annotation required on all exposed methods. If null, no annotation is required. In practice,
   * this is usually {@link android.webkit.JavascriptInterface}.
   */
   ...
   *
   * <p>Cannot be held strongly, because it may (via the objects it holds) contain references which
   * form an uncollectable cycle.
   */
  ...
  public void invokeMethod(
    ...
    for (int i = 0; i < numArguments; i++) {
      try {
        args[i] =
            convertArgument(
                arguments[i], parameterTypes[i], StringCoercionMode.COERCE, objectIdAllocator);
    ...
    RemoteInvocationResult mojoResult =
        convertResult(result, method.getReturnType(), objectIdAllocator, mSafeAnnotationClass);
      ...
      case RemoteInvocationArgument.Tag.TypedArrayValue:
        ...
        if (parameterType.isArray()) {
          ...
          ByteBuffer typedBuffer =
              ByteBuffer.wrap(BigBufferUtil.getBytesFromBigBuffer(typedArrayValue.buffer));
          ...
          if (typedArrayValue.type == RemoteArrayType.INT8_ARRAY) {
            ...
            return new WrapShortBuffer(typedBuffer.asShortBuffer(), componentType, false)
                .copyArray();
  ...
  /**
   * Returns whether the value is an Int32 in the V8 API sense. That is, it has an integer value in
   * [-2^31, 2^31) and is not negative zero.
   */
