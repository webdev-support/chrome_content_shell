...
public class ContentUiEventHandler implements UserData {
  ...
  /**
   * Check whether a key should be propagated to the embedder or not. We need to send almost every
   * key to Blink. However: 1. We don't want to block the device on the renderer for some keys like
   * menu, home, call. 2. There are no WebKit equivalents for some of these keys (see
   * app/keyboard_codes_win.h) Note that these are not the same set as KeyEvent.isSystemKey: for
   * instance, AKEYCODE_MEDIA_* will be dispatched to webkit*. 3. KEYCODE_SYSRQ is the keycode
   * received when Print screen is pressed on a hardware keyboard. Do not propagate this to allow
   * Android Platform to take a screenshot.
   */
  ...
  /**
   * @see View#scrollBy(int, int) Currently the ContentView scrolling happens in the native side. In
   *     the Java view system, it is always pinned at (0, 0). scrollBy() and scrollTo() are
   *     overridden, so that View's mScrollX and mScrollY will be unchanged at (0, 0). This is
   *     critical for drawing ContentView correctly.
   */
  @CalledByNative
  ...
  interface Natives {
    ...
    void cancelFling(long nativeContentUiEventHandler, ContentUiEventHandler caller, long timeMs);
  }
}
