...
/**
 * Tracks the Activiy for a given WebContents on behalf of a NFC instance that cannot talk directly
 * to WebContents.
 */
class NfcHost implements WindowEventObserver {
  ...
  /**
   * Called by the NFC implementation (via ContentNfcDelegate) to allow that implementation to track
   * changes to the Activity associated with its context ID (i.e., the activity associated with
   * |mWebContents|).
   */
