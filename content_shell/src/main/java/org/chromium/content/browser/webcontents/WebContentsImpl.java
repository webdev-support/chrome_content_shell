...
public class WebContentsImpl implements WebContents, RenderFrameHostDelegate, WindowEventObserver {
  ...
  /**
   * Used to reset the internal tracking for whether or not a serialized {@link WebContents} was
   * created in this process or not.
   */
  ...
  /**
   * A {@link android.os.Parcelable.Creator} instance that is used to build {@link WebContentsImpl}
   * objects from a {@link Parcel}.
   */
  ...
  public static final Parcelable.Creator<WebContents> CREATOR =
      new Parcelable.Creator<WebContents>() {
        ...
        public WebContents createFromParcel(Parcel source) {
          ...
          return WebContentsImplJni.get().fromNativePtr(bundle.getLong(PARCEL_WEBCONTENTS_KEY));
        }
  ...
  public RenderFrameHost getRenderFrameHostFromId(GlobalRenderFrameHostId id) {
    ...
    return WebContentsImplJni.get()
        .getRenderFrameHostFromId(mNativeWebContentsAndroid, id.childId(), id.frameRoutingId());
  }
  ...
  /**
   * Thrown by reportDanglingPtrToBrowserContext(), indicating that WebContentsImpl is deleted after
   * its BrowserContext.
   */
  ...
  private static void reportDanglingPtrToBrowserContext(Throwable creator) {
    JavaExceptionReporter.reportException(
        new DanglingPointerException("Dangling pointer to BrowserContext in WebContents", creator));
  }
  ...
  public void setImportance(@ChildProcessImportance int primaryMainFrameImportance) {
    checkNotDestroyed();
    WebContentsImplJni.get().setImportance(mNativeWebContentsAndroid, primaryMainFrameImportance);
  }
  ...
  public void addMessageToDevToolsConsole(int level, String message) {
    checkNotDestroyed();
    WebContentsImplJni.get().addMessageToDevToolsConsole(mNativeWebContentsAndroid, level, message);
  }
  ...
  /**
   * Requests a snapshop of accessibility tree. The result is provided asynchronously using the
   * callback
   *
   * @param callback The callback to be called when the snapshot is ready. The callback cannot be
   *     null.
   */
  public void requestAccessibilitySnapshot(ViewStructure root, Runnable doneCallback) {
    ...
    WebContentsImplJni.get()
        .requestAccessibilitySnapshot(mNativeWebContentsAndroid, root, builder, doneCallback);
  }
  ...
  public void setStylusWritingHandler(StylusWritingHandler stylusWritingHandler) {
    ...
    if (mNativeWebContentsAndroid == 0) return;
    WebContentsImplJni.get()
        .setStylusHandwritingEnabled(mNativeWebContentsAndroid, mStylusWritingHandler != null);
  }
  ...
  public boolean hasActiveEffectivelyFullscreenVideo() {
    checkNotDestroyed();
    return WebContentsImplJni.get().hasActiveEffectivelyFullscreenVideo(mNativeWebContentsAndroid);
  }
  ...
  private final void setMediaSession(MediaSessionImpl mediaSession) {
    ...
    mediaSession.addObserver(
        new MediaSessionObserver(mediaSession) {
          @Override
          public void mediaSessionDestroyed() {
            super.mediaSessionDestroyed();
          }

          @Override
          public void mediaSessionStateChanged(boolean isControllable, boolean isSuspended) {
            super.mediaSessionStateChanged(isControllable, isSuspended);
          }

          @Override
          public void mediaSessionMetadataChanged(MediaMetadata metadata) {
            super.mediaSessionMetadataChanged(metadata);
          }

          @Override
          public void mediaSessionActionsChanged(Set<Integer> actions) {
            super.mediaSessionActionsChanged(actions);
          }

          @Override
          public void mediaSessionArtworkChanged(List<MediaImage> images) {
            super.mediaSessionArtworkChanged(images);
          }

          @Override
          public void mediaSessionPositionChanged(@Nullable MediaPosition position) {
            super.mediaSessionPositionChanged(position);
          }
        });
  }
  ...
  /**
   * @return {@code UserDataHost} that contains internal user data. {@code null} if it is already
   *     gc'ed.
   */
  ...
  public void onRotationChanged(int rotation) {
    ...
    WebContentsImplJni.get().sendOrientationChangeEvent(mNativeWebContentsAndroid, rotationDegrees);
  }
  ...
  public boolean needToFireBeforeUnloadOrUnloadEvents() {
    ...
    return WebContentsImplJni.get().needToFireBeforeUnloadOrUnloadEvents(mNativeWebContentsAndroid);
  }
  ...
  public interface Natives {
    ...
    void setViewAndroidDelegate(long nativeWebContentsAndroid, ViewAndroidDelegate viewDelegate);
