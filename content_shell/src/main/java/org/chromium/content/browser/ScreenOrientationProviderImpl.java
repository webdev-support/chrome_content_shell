...
public class ScreenOrientationProviderImpl
  ...
  /**
   * The keys of the map are the activities for which screen orientation are trying to lock. The
   * values of the map are the most recent default web screen orientation request for each activity.
   */
  private Map<Activity, Byte> mDefaultOrientationOverrides = new WeakHashMap<>();

  /**
   * The keys of the map are the activities for which screen orientation requests are delayed. The
   * values of the map are the most recent screen orientation request for each activity. The map
   * will contain an entry with a null value if screen orientation requests are delayed for an
   * activity but no screen orientation requests have been made for the activity.
   */
    ...
    switch (orientation) {
      ...
      case ScreenOrientationLockType.NATURAL:
        ...
        DisplayAndroid displayAndroid =
            (window != null) ? window.getDisplay() : DisplayAndroid.getNonMultiDisplay(context);
  ...
  private void addPendingRequest(
    ...
    WindowEventObserverManager windowEventManager = WindowEventObserverManager.from(webContents);
