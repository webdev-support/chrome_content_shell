...
public class X509Util {
  ...
  /**
   * Trust manager backed up by a custom certificate store. We need such manager to plant test root
   * CA to the trust store in testing.
   */
  ...
  /**
   * The system key store. This is used to determine whether a trust anchor is a system trust anchor
   * or user-installed.
   */
  private static KeyStore sSystemKeyStore;

  /**
   * The directory where system certificates are stored. This is used to determine whether a trust
   * anchor is a system trust anchor or user-installed. The KeyStore API alone is not sufficient to
   * efficiently query whether a given X500Principal, PublicKey pair is a trust anchor.
   */
  ...
  /**
   * True if the system key store has been loaded. If the "AndroidCAStore" KeyStore instance was not
   * found, sSystemKeyStore may be null while sLoadedSystemKeyStore is true.
   */
  ...
  /**
   * Ensures that test trust managers and certificate factory are initialized. Must be called with
   * |sLock| held.
   */
  ...
  /**
   * Creates a X509TrustManagerExtensions backed up by the given key store. When null is passed as a
   * key store, system default trust store is used. Returns null if no created TrustManager was
   * suitable.
   *
  ...
  /**
   * If an EKU extension is present in the end-entity certificate, it MUST contain either the anyEKU
   * or serverAuth or netscapeSGC or Microsoft SGC EKUs.
   *
