...
class AndroidNetworkLibrary {
  ...
  /**
   * @return the mime type (if any) that is associated with the file extension. Returns null if no
   *     corresponding mime type exists.
   */
  @CalledByNative
  ...
  /**
   * @return true if it can determine that only loopback addresses are configured. i.e. if only
   *     127.0.0.1 and ::1 are routable. Also returns false if it cannot determine this.
   */
  @CalledByNative
  ...
  /**
   * Validate the server's certificate chain is trusted. Note that the caller must still verify the
   * name matches that of the leaf certificate.
   *
  ...
  /**
   * Removes all test root certificates added by |addTestRootCertificate| calls from the local trust
   * store.
   */
  @CalledByNativeUnchecked
  ...
  /**
   * Returns the MCC+MNC (mobile country code + mobile network code) as the numeric name of the
   * current registered operator. This function potentially blocks the thread, so use with care.
   */
  @CalledByNative
  private static String getNetworkOperator() {
    TelephonyManager telephonyManager =
        (TelephonyManager)
            ContextUtils.getApplicationContext().getSystemService(Context.TELEPHONY_SERVICE);
  ...
  private static boolean getIsRoaming() {
    ConnectivityManager connectivityManager =
        (ConnectivityManager)
            ContextUtils.getApplicationContext().getSystemService(Context.CONNECTIVITY_SERVICE);
   ...
   * process to the captive portal has been successfully completed, or if the captive portal status
   * can't be determined. Requires ACCESS_NETWORK_STATE permission. Only available on Android
  ...
  /**
   * Helper function that gets the WifiInfo of the WiFi network. If we have permission to access to
   * the WiFi state, then we use either {@link NetworkCapabilities} for Android S+ or {@link
  ...
  public static void setWifiEnabledForTesting(boolean enabled) {
    WifiManager wifiManager =
        (WifiManager) ContextUtils.getApplicationContext().getSystemService(Context.WIFI_SERVICE);
    boolean wifiEnabled = wifiManager.isWifiEnabled();
    wifiManager.setWifiEnabled(enabled);
    ResettersForTesting.register(() -> wifiManager.setWifiEnabled(wifiEnabled));
  }
  ...
  /** Returns object representing the DNS configuration for the provided network handle. */
  ...
  /** Returns object representing the DNS configuration for the current default network. */
  @RequiresApi(Build.VERSION_CODES.M)
  @CalledByNative
  ...
  /**
   * Returns object representing the DNS configuration for the provided network. If |network| is
   * null, uses the active network.
   */
  @RequiresApi(Build.VERSION_CODES.M)
  ...
  /**
   * Class to wrap FileDescriptor.setInt$() which is hidden and so must be accessed via reflection.
   */
  ...
  /**
   * This class provides an implementation of {@link java.net.Socket} that serves only as a conduit
   * to pass a file descriptor integer to {@link android.net.TrafficStats#tagSocket} when called by
   * {@link #tagSocket}. This class does not take ownership of the file descriptor, so calling
   * {@link #close} will not actually close the file descriptor.
   */
  private static class SocketFd extends Socket {
    /**
     * This class provides an implementation of {@link java.net.SocketImpl} that serves only as a
     * conduit to pass a file descriptor integer to {@link android.net.TrafficStats#tagSocket} when
     * called by {@link #tagSocket}. This class does not take ownership of the file descriptor, so
     * calling {@link #close} will not actually close the file descriptor.
     */
    private static class SocketImplFd extends SocketImpl {
       ...
       * descriptor. Does not take ownership of the file descriptor, so calling {@link #close} will
       * not actually close the file descriptor.
       */
    ...
    /**
     * Create a {@link java.net.Socket} that sets {@code fd} as the underlying file descriptor. Does
     * not take ownership of the file descriptor, so calling {@link #close} will not actually close
     * the file descriptor.
     */
   ...
   *
   * <p>Assumes thread UID tag isn't set upon entry, and ensures thread UID tag isn't set upon exit.
