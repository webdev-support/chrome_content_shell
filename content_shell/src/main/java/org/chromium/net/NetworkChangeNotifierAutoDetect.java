...
public class NetworkChangeNotifierAutoDetect extends BroadcastReceiver {
  ...
  static class ConnectivityManagerDelegate {
    ...
    void registerNetworkCallback(
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        try (StrictModeContext ignored = StrictModeContext.allowDiskReads()) {
          ...
          mConnectivityManager.registerNetworkCallback(networkRequest, networkCallback, handler);
        }
      } else {
    ...
    void registerDefaultNetworkCallback(NetworkCallback networkCallback, Handler handler) {
      ApiHelperForO.registerDefaultNetworkCallback(mConnectivityManager, networkCallback, handler);
    }
  ...
  static class WifiManagerDelegate {
    ...
    private boolean hasPermissionLocked() {
      ...
      mHasWifiPermission =
          mContext
                  .getPackageManager()
                  .checkPermission(permission.ACCESS_WIFI_STATE, mContext.getPackageName())
              == PackageManager.PERMISSION_GRANTED;
      // TODO(crbug.com/635567): Fix lint properly.
      mWifiManager =
          mHasWifiPermission ? (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE) : null;
  ...
  private class MyNetworkCallback extends NetworkCallback {
    ...
    void initializeVpnInPlace() {
      try (ScopedSysTraceEvent event =
          ScopedSysTraceEvent.scoped("NetworkChangeNotifierAutoDetect.initializeVpnInPlace")) {
    ...
    /**
     * Should changes to network {@code network} be ignored due to a VPN being in place and blocking
     * direct access to {@code network}?
     *
    ...
    public void onLost(final Network network) {
      try (TraceEvent e = TraceEvent.scoped("NetworkChangeNotifierCallback::onLost")) {
        ...
        if (mVpnInPlace != null) {
          ...
          mVpnInPlace = null;
          for (Network newNetwork : getAllNetworksFiltered(mConnectivityManagerDelegate, network)) {
  ...
  /**
   * Abstract class for providing a policy regarding when the NetworkChangeNotifier should listen
   * for network changes.
   */
  public abstract static class RegistrationPolicy {
    ...
    /**
     * Initializes the policy with the notifier, overriding subclasses should always call this
     * method.
     */
  ...
  public static interface Observer {
    ...
    /**
     * Called when device connects to network with NetID netId. For example device associates with a
     * WiFi access point. connectionType is the type of the network; a member of ConnectionType.
     ...
     * cellular connection no longer preferred. The device will disconnect from the network in 30s
     * allowing network communications on that network to wrap up. Only called on Android L and
     * above.
     */
    public void onNetworkSoonToDisconnect(long netId);

    /**
     * Called when device disconnects from network with NetID netId. Only called on Android L and
     * above.
     */
     ...
     * accompanying list of active networks. This is issued if a period elapsed where disconnected
     * notifications may have been missed, and acts to keep cached lists of active networks
   ...
   * thread.
   *
   * @param policy The RegistrationPolicy which determines when this class should watch for network
   *     changes (e.g. see (@link RegistrationPolicyAlwaysRegister} and {@link
    ...
    try (ScopedSysTraceEvent event =
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
        ...
        mDefaultNetworkCallback =
            Build.VERSION.SDK_INT >= Build.VERSION_CODES.P ? new DefaultNetworkCallback() : null;
      }
      updateCurrentNetworkState();
  ...
  void setConnectivityManagerDelegateForTests(ConnectivityManagerDelegate delegate) {
    ConnectivityManagerDelegate oldValue = mConnectivityManagerDelegate;
  ...
  void setWifiManagerDelegateForTests(WifiManagerDelegate delegate) {
    WifiManagerDelegate oldValue = mWifiManagerDelegate;
   ...
   * TODO(crbug/1493005): migrate external callers to getCurrentNetworkState() and make this method
   * private (to be called only when updates are received from the system.)
   */
  public void updateCurrentNetworkState() {
    try (ScopedSysTraceEvent event =
        ScopedSysTraceEvent.scoped("NetworkChangeNotifierAutoDetect.updateCurrentNetworkState")) {
  ...
  public NetworkState getCurrentNetworkState() {
    try (ScopedSysTraceEvent event =
        ScopedSysTraceEvent.scoped("NetworkChangeNotifierAutoDetect.getCurrentNetworkState")) {
  ...
  /**
   * Returns an array of all of the device's currently connected networks and ConnectionTypes,
   * including only those that are useful and accessible to Chrome. Array elements are a repeated
   * sequence of: NetID of network ConnectionType of network Only available when auto-detection has
   * been enabled.
   */
  ...
  /**
   * Returns the device's current default connected network used for communication. Returns null
  ...
  /**
   * Returns NetID of device's current default connected network used for communication. Returns
   * NetId.INVALID when not implemented.
   */
  ...
  private void connectionTypeChanged() {
    try (ScopedSysTraceEvent event =
        ScopedSysTraceEvent.scoped("NetworkChangeNotifierAutoDetect.connectionTypeChanged")) {
      connectionTypeChangedTo(mConnectivityManagerDelegate.getNetworkState(mWifiManagerDelegate));
    }
  }
  ...
  /**
   * Extracts NetID of Network on Lollipop and NetworkHandle (which is munged NetID) on Marshmallow
   * and newer releases. TODO(crbug.com/1489183): Rename networkToNetId to something meaningful and
