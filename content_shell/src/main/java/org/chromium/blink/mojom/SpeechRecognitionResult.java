package org.chromium.blink.mojom;

import java.nio.ByteBuffer;
import java.util.ArrayList;
import org.chromium.mojo.bindings.DataHeader;
import org.chromium.mojo.bindings.Decoder;
import org.chromium.mojo.bindings.Encoder;
import org.chromium.mojo.bindings.Message;
import org.chromium.mojo.bindings.Struct;

public final class SpeechRecognitionResult extends Struct {
  private static final DataHeader DEFAULT_STRUCT_INFO;
  private static final int STRUCT_SIZE = 24;
  private static final DataHeader[] VERSION_ARRAY;
  public SpeechRecognitionHypothesis[] hypotheses;
  public boolean isProvisional;

  static {
    DataHeader[] dataHeaderArr = {new DataHeader(24, 0)};
    VERSION_ARRAY = dataHeaderArr;
    DEFAULT_STRUCT_INFO = dataHeaderArr[0];
  }

  private SpeechRecognitionResult(int version) {
    super(24, version);
  }

  public SpeechRecognitionResult() {
    this(0);
  }

  public static SpeechRecognitionResult deserialize(Message message) {
    return decode(new Decoder(message));
  }

  public static SpeechRecognitionResult deserialize(ByteBuffer data) {
    return deserialize(new Message(data, new ArrayList()));
  }

  public static SpeechRecognitionResult decode(Decoder decoder0) {
    if (decoder0 == null) {
      return null;
    }
    decoder0.increaseStackDepth();
    try {
      DataHeader mainDataHeader = decoder0.readAndValidateDataHeader(VERSION_ARRAY);
      int elementsOrVersion = mainDataHeader.elementsOrVersion;
      SpeechRecognitionResult result = new SpeechRecognitionResult(elementsOrVersion);
      Decoder decoder1 = decoder0.readPointer(8, false);
      DataHeader si1 = decoder1.readDataHeaderForPointerArray(-1);
      result.hypotheses = new SpeechRecognitionHypothesis[si1.elementsOrVersion];
      for (int i1 = 0; i1 < si1.elementsOrVersion; i1++) {
        Decoder decoder2 = decoder1.readPointer((i1 * 8) + 8, false);
        result.hypotheses[i1] = SpeechRecognitionHypothesis.decode(decoder2);
      }
      result.isProvisional = decoder0.readBoolean(16, 0);
      return result;
    } finally {
      decoder0.decreaseStackDepth();
    }
  }

  @Override
  public final void encode(Encoder encoder) {
    Encoder encoder0 = encoder.getEncoderAtDataOffset(DEFAULT_STRUCT_INFO);
    SpeechRecognitionHypothesis[] speechRecognitionHypothesisArr = this.hypotheses;
    if (speechRecognitionHypothesisArr == null) {
      encoder0.encodeNullPointer(8, false);
    } else {
      Encoder encoder1 = encoder0.encodePointerArray(speechRecognitionHypothesisArr.length, 8, -1);
      int i0 = 0;
      while (true) {
        SpeechRecognitionHypothesis[] speechRecognitionHypothesisArr2 = this.hypotheses;
        if (i0 >= speechRecognitionHypothesisArr2.length) {
          break;
        }
        encoder1.encode((Struct) speechRecognitionHypothesisArr2[i0], (i0 * 8) + 8, false);
        i0++;
      }
    }
    encoder0.encode(this.isProvisional, 16, 0);
  }
}
