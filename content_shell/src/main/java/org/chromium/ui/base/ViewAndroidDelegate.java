...
public class ViewAndroidDelegate {
  ...
  /**
   * Adds observer that needs notification when container view is updated. Note that there is no
   * {@code removObserver} since the added observers are all supposed to go away with this object
   * together.
   *
   * @param observer {@link ContainerViewObserver} object. The object should have the lifetime same
   *     as this {@link ViewAndroidDelegate} to avoid gc issues.
   */
   ...
   *
   * <p>WARNING: This method can also be used to replace the existing container view, but you should
   * only do it if you have a very good reason to. Replacing the container view has been designed to
   * support fullscreen in the Webview so it might not be appropriate for other use cases.
   *
  ...
  /**
   * Get the tracker that records the drag event on the view this delegate attached to. Will return
   * null if there's no {@link DragStateTracker} set up.
   */
  ...
  public void setViewPosition(
      View anchorView, float x, float y, float width, float height, int leftMargin, int topMargin) {
    ViewGroup containerView = getContainerViewGroup();
    if (containerView == null) return;
   ...
   * @param current_scroll_ratio the ratio of vertical scroll in [0, 1] range. Scroll at top of page
   ...
   * container view is usually ContentView, and ContentView only supports children directly added by
   * this class. See ContentView for details on this.
   *
