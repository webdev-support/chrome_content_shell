...
public class EventForwarder {
  ...
  EventForwarder(
      long nativeEventForwarder, boolean isDragDropEnabled, boolean convertTrackpadEventsToMouse) {
  ...
  /**
   * Creates a new motion event differed from the given event by current touch offset if the offset
   * is not zero.
   *
   * <p>Sets the current amount to offset incoming drag events by. Used for {@link DragEvent} only.
  ...
  /**
   * Sends mouse event to native. Hover event is also converted to mouse event, only differentiated
   * by an internal flag.
   */
  private boolean sendNativeMouseEvent(MotionEvent event) {
    ...
    boolean shouldConvertToMouseEvent =
        isTrackpadToMouseEventConversionEnabled() && isTrackpadClickOrClickAndDragEvent(event);
    EventForwarderJni.get()
        .onMouseEvent(
            mNativeEventForwarder,
            EventForwarder.this,
  ...
  /**
   * Returns true if a {@link MotionEvent} is a trackpad click and or click & drag event. Trackpad
   * hover events and non-click gestures (i.e two-finger scroll) should return false here as they do
   * have an action button pressed. Also we want to make sure we return true for button release
   * events as well.
   */
  public static boolean isTrackpadClickOrClickAndDragEvent(MotionEvent event) {
    return isTrackpadEvent(event)
        && (event.getAction() == MotionEvent.ACTION_BUTTON_RELEASE || event.getButtonState() != 0);
  }

  /**
   * Returns true if a {@link MotionEvent} is detected to be a trackpad event. Note that {@link
   * MotionEvent.TOOL_TYPE_FINGER} is used here along with {@link InputDevice.SOURCE_MOUSE} instead
   * of {@link InputDevice.SOURCE_TOUCHPAD} because {@link InputDevice.SOURCE_TOUCHPAD} is used when
   * an app captures the touchpad meaning that it gets access to the raw finger locations,
   * dimensions etc. reported by the touchpad rather than those being used for pointer movements and
   * gestures.
   */
  ...
  public boolean onDragEvent(DragEvent event, View containerView) {
    ...
    if (clipDescription != null && clipDescription.hasMimeType(MimeTypeUtils.CHROME_MIMETYPE_TAB)) {
      return false;
    }
   ...
   * @param syntheticScroll true if generated by gamepad (which will make this fixed-velocity fling)
  ...
  interface Natives {
    ...
    boolean onKeyUp(long nativeEventForwarder, EventForwarder caller, KeyEvent event, int keyCode);
