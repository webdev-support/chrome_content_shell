 ...
 * below).
 *
 * <p>{@link #build()} will instantiate AnimatedStateListDrawable on platforms where it is supported
 * (API level 21+). On older APIs, transition animations will be ignored and StateListDrawable will
 * be instantiated instead.
 *
 * <p>Usage: StateListDrawableBuilder builder = new StateListDrawableBuilder(context);
 * StateListDrawableBuilder.State checked = builder.addState(R.drawable.checked,
 * android.R.attr.state_checked); StateListDrawableBuilder.State unchecked =
 * builder.addState(R.drawable.unchecked); builder.addTransition(checked, unchecked,
 * R.drawable.transition_checked_unchecked); builder.addTransition(unchecked, checked,
 * R.drawable.transition_unchecked_checked); StateListDrawable drawable = builder.build();
 */
public class StateListDrawableBuilder {
   ...
   * @param stateSet Array of state ids that specify the state. See {@link android.R.attr} for the
   *     list of state ids provided by the platform.
   */
  ...
  public StateListDrawable build() {
    ...
    for (int i = 0; i < transitionsSize; ++i) {
      ...
      result.addTransition(
          transition.getFromId(), transition.getToId(), (Drawable & Animatable) (drawable), false);
    }
    return result;
  }
}
