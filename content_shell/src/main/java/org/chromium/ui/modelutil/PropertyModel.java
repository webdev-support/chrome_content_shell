...
public class PropertyModel extends PropertyObservable<PropertyKey> {
   ...
   *
   * <ul>
   *   <li>In a RecyclerView where you want to defer expensive or memory intensive operations until
   *       it is needed to display on screen.
  ...
  /**
   * Returns the value from the item model based on the key. Otherwise returns the passed in default
   * value.
   *
   * @param model The model for the list menu item.
   * @param key The key of the property to retrieve.
  ...
  public static class Builder {
    ...
    /**
     * Adds a transforming key. The passed in {@link ReadableTransformingObjectPropertyKey} must not
     * already exist in the set of keys.
     *
     ...
     */
    public <T, V> Builder withTransformingKey(
        ReadableTransformingObjectPropertyKey<T, V> key, Function<T, V> transformer, T value) {
