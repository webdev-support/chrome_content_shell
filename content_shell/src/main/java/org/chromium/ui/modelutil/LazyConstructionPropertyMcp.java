...
/**
 * ModelChangeProcessor for property-observable models that can be visible or hidden. Lazily creates
 * the view the first time the model is shown, and queues up property changes while the model is
 * hidden, dispatching them when it is shown.
 *
...
public class LazyConstructionPropertyMcp<M extends PropertyObservable<P>, V, P>
  ...
  public static <M extends PropertyModel, V> LazyConstructionPropertyMcp<M, V, PropertyKey> create(
      M model,
    ...
    return new LazyConstructionPropertyMcp<>(
        model, visibilityProperty, item -> item.get(visibilityProperty), viewFactory, viewBinder);
  }
