...
public class RecyclerViewAdapter<VH extends ViewHolder, P> extends RecyclerView.Adapter<VH>
  ...
  public interface Delegate<VH, P> extends ListObservable<P> {
    /**
     * Returns the number of items under this subtree. This method may be called before
     * initialization.
     *
    ...
    /**
     * Bind a given {@code viewHolder} to the data represented by the item at the given {@code
     * position} in the adapter. If {@code payload} is non-null, performs a "partial" update of only
     * the property represented by {@code payload}.
     *
    ...
    /**
     * Called when the {@link View} owned by {@code viewHolder} no longer needs to be attached to
     * its parent {@link RecyclerView}. This is a good place to free up expensive resources that
     * might be owned by the particular {@link View} or {@code viewHolder}.
     *
     ...
     * dumping state and equality checks, different items should have distinct descriptions, but for
     * items that are unique or don't have interesting state it can be sufficient to return e.g. a
     * string that describes the type of the item.
     *
  ...
  public interface ViewHolderFactory<VH> {
    /**
     * Called when the {@link RecyclerView} needs a new {@link ViewHolder} of the given {@code
     * viewType} to represent an item.
     *
     * @param parent The {@link ViewGroup} into which the new view will be added after it's bound to
     *     an adapter position.
