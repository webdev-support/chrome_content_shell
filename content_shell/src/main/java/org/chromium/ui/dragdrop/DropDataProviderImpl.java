 ...
 *     <p>TODO(https://crbug.com/1353048): Add the reference to //android_webview/support_library
 *     content provider to this java doc.
 */
...
public class DropDataProviderImpl {
  ...
  private static class DropPipeDataWriter implements ContentProvider.PipeDataWriter<byte[]> {
    ...
    public void writeDataToPipe(
        ParcelFileDescriptor output, Uri uri, String mimeType, Bundle opts, byte[] imageBytes) {
  ...
  private Uri generateUri() {
    ...
    return new Uri.Builder()
        ...
        .authority(ContextUtils.getApplicationContext().getPackageName() + URI_AUTHORITY_SUFFIX)
        .path(timestamp)
        .build();
  }
   ...
   *
   * @param imageInUse Indicate if the image is needed by the drop target app. This is true when the
   *     image is dropped outside of Chrome AND the drop target app returns true for event
   *     ACTION_DROP.
   */
    ...
    switch (method) {
      ...
      case SET_INTERVAL_METHOD_NAME:
        setClearCachedDataIntervalMs(
            extras.getInt(
                CLEAR_CACHE_PARAM, DropDataProviderImpl.DEFAULT_CLEAR_CACHED_DATA_INTERVAL_MS));
        break;
