...
public class DragDropGlobalState {
  ...
  public static final class TrackerToken {
    ...
    public boolean equals(@Nullable Object obj) {
      if (!(obj instanceof TrackerToken)) {
        return false;
      }
      TrackerToken that = ((TrackerToken) obj);
      return that.mKey == this.mKey;
    }
  }

  /**
   * Build a {@link DragDropGlobalState} to be stored in the static instance, and return the {@link
   * TrackerToken} to retrieve and modify the state.
   *
  ...
  public static TrackerToken store(
    ...
    if (sGlobalStateHolder != null) {
      Log.w(TAG, "Holder isn't cleared before store. " + sGlobalStateHolder.mInstance.toString());
    }
    sGlobalStateHolder =
  ...
  /**
   * Tokens are released when startDragAndDrop fails or by listeners on drag end event. If a caller
   * fails to release token, sHolder is not cleared and the next build call will fail.
   */
