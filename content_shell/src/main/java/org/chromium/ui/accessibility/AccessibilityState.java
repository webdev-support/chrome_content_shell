...
public class AccessibilityState {
  ...
  public interface Listener {
     ...
     * example when a user: - enables/disables an accessibility service (e.g. TalkBack, VoiceAccess,
     * etc.) - enables/disables a pseudo-accessibility service (e.g. password manager, etc.) -
     * changes an accessibility-related system setting (e.g. animation duration, password obscuring,
     * touch exploration, etc.)
     *
     * <p>For a full list of triggers, see: {AccessibilityState#registerObservers} For a full list
     * of tracked settings, see: {AccessibilityState.State}
     *
     * <p>This method passes both the previous and new (old current vs. now current) accessibility
  ...
  /**
   * True when at least one accessibility service is enabled on the system. Since a client can call
   * this after observers are registered, but before the State has been queried for the first time,
   * we allow for an early return. This is a lighter weight query than the other State booleans,
   * which require manual calculation and heuristics. In this case we return the value directly from
   * AccessibilityManager.
   *
   ...
   * default font weight. If a user has the bold text setting enabled, this will be 300. This is not
   * included as a part of the {State} object since it is only needed for the web contents rendering
   * (native widgets have font weight adjusted by the framework). This is only available on Android
   * S+, on previous versions of Android this is always 0.
   */
   ...
   *
   * <p>Avoid calling this method at all costs. The naming of this method is misleading and its
   * usage is tricky. Use the more granular methods of this class.
   *
   * <p>Returns true if an accessibility service is running that uses touch exploration OR a service
  ...
  /**
   * Convenience method to get a recommended timeout on all versions of Android. The method that is
   * part of AccessibilityManager is only available on Android >= Q. For earlier versions of
   ...
   * services, to provide a suggested timeout. On Android >= Q, the returned value may not be either
   * of the provided timeouts, and for versions < Q this will return the maximum of the two
   * timeouts.
   *
   ...
   * requiring a hard dependency on AccessibilityManager or an instance of a View. If this method is
   * called when accessibility has been disabled (e.g. stale state after calling off the main
   * thread), then the event will be ignored. If an event is sent, this does not guarantee a correct
   * user experience for downstream AT.
   *
   * <p>Note: This should only be used in exceptional situations. Apps can generally achieve the
   * correct behavior for accessibility with a semantically correct UI. Deprecated to prompt dev to
  ...
  private static void fetchAccessibilityManager() {
    ...
    sAccessibilityManager =
        (AccessibilityManager)
            ContextUtils.getApplicationContext().getSystemService(Context.ACCESSIBILITY_SERVICE);
  }
  ...
  protected static void updateAccessibilityServices() {
    ...
    for (AccessibilityServiceInfo service : serviceInfoList) {
      ...
      isAccessibilityToolPresent |=
          (Build.VERSION.SDK_INT < Build.VERSION_CODES.S || service.isAccessibilityTool());
    ...
    int textShowPasswordSetting =
        Settings.System.getInt(context.getContentResolver(), Settings.System.TEXT_SHOW_PASSWORD, 1);
    ...
    if (runningServiceNames.equals(enabledServiceNames)) {
      ...
      if (sNextDelayMillis < MAX_DELAY_MILLIS) {
        ...
        RecordHistogram.recordBooleanHistogram(UPDATE_ACCESSIBILITY_SERVICES_DID_POLL, true);
        ThreadUtils.getUiThreadHandler()
            .postDelayed(AccessibilityState::updateAccessibilityServices, sNextDelayMillis);
        sPollCount++;
        sNextDelayMillis *= 2;
        return;
      } else {
        Log.i(TAG, "Max delay reached. Send information as is.");
        RecordHistogram.recordBooleanHistogram(UPDATE_ACCESSIBILITY_SERVICES_POLL_TIMEOUT, true);
      ...
      if (autofillManager != null
        ...
        if (runningServiceNames.isEmpty()
            || (runningServiceNames.size() == 1
                && runningServiceNames.get(0).equals(AUTOFILL_COMPAT_ACCESSIBILITY_SERVICE_ID))) {
    ...
    boolean isOnlyPasswordManagersEnabled;
    boolean areOnlyPasswordManagerMasksRequestedByServices = areOnlyPasswordManagerMasksRequested();
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
    ...
    boolean isPerformGesturesEnabled = (0 != (sCapabilitiesMask & CAPABILITY_CAN_PERFORM_GESTURES));
  ...
  /**
   * Return a bitmask containing the union of all feedback types that running accessibility services
   * provide.
   *
   * @return
   */
   ...
   *
   * <p>Note: This should only be called once, and before any client queries of accessibility state.
  ...
  public static void setStateMaskForTesting(StateIdentifierForTesting state, int value) {
    ...
    if (state == EVENT_TYPE_MASK) {
      sEventTypeMask = value;
    } else if (state == FEEDBACK_TYPE_MASK) {
      sFeedbackTypeMask = value;
    } else if (state == FLAGS_MASK) {
      sFlagsMask = value;
    } else if (state == CAPABILITIES_MASK) {
      sCapabilitiesMask = value;
    } else if (state == EVENT_TYPE_MASK_HEURISTIC) {
      sEventTypeMaskHeuristic = value;
    } else if (state == FEEDBACK_TYPE_MASK_HEURISTIC) {
      sFeedbackTypeMaskHeuristic = value;
    } else if (state == FLAGS_MASK_HEURISTIC) {
      sFlagsMaskHeuristic = value;
    } else if (state == CAPABILITIES_MASK_HEURISTIC) {
      sCapabilitiesMaskHeuristic = value;
    }
  }
