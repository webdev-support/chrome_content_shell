...
public class DynamicResourceLoader extends ResourceLoader {
  /**
   * Adapter for holding a callback and {@link DynamicResource}. The callback must be held so it can
   * be unregistered when the {@link DynamicResource} is no longer in use.
   */
   ...
   * @param callback A {@link ResourceLoaderCallback} to be notified when the dynamic resource has
   *     changed. The callback will only be notified if {@link #loadResource(int)} is called.
   */
