...
public class HardwareDraw implements CaptureMechanism {
  ...
  @IntDef({
    ImageReaderStatus.NEW,
    ImageReaderStatus.INITIALIZING,
    ImageReaderStatus.UPDATED,
  ...
  private static class RequestState {
    ...
    private RequestState(int requestId, View view, float scale, Callback<Bitmap> onBitmapCapture) {
  ...
  private class AcceleratedImageReader implements ImageReader.OnImageAvailableListener {
    ...
    private void init(int width, int height) {
      try (TraceEvent e = TraceEvent.scoped("AcceleratedImageReader::init")) {
        ...
        mReaderDelegate =
            ImageReader.newInstance(width, height, PixelFormat.RGBA_8888, maxBitmapsToAcquire);
    ...
    public void onImageAvailable(ImageReader reader) {
      try (TraceEvent e = TraceEvent.scoped("AcceleratedImageReader::onImageAvailable")) {
        ...
        AsyncTask asyncTask =
            new AsyncTask<State>() {
              @Override
              protected State doInBackground() {
                try (TraceEvent e2 =
                    TraceEvent.scoped("AcceleratedImageReader::onImageAvailable::postTask")) {
                  ...
                  // Set |mHardwareBitmap| to use the correct padding so it will
                  // copy
                  // from the buffer.
                  ...
                  return new State(width, height, rowPaddingInPixels, snapshot);
                }
              }

              @Override
              protected void onPostExecute(State currentState) {
                mState = currentState;
                int scaledWidth = (int) (requestState.view.getWidth() * requestState.scale);
                int scaledHeight = (int) (requestState.view.getHeight() * requestState.scale);
        ...
        asyncTask.executeOnTaskRunner(mTaskRunner);
      }
    }
  ...
  @Nullable private AcceleratedImageReader mReader;
  ...
  /**
   * Each instance should be called by external clients only on the thread it is created. The first
   * instance created will also create a thread to do the actual rendering on.
   */
  public HardwareDraw() {
    mThreadChecker.assertOnValidThread();
    if (sHandler == null) {
  ...
  private boolean captureHardware(
      Canvas canvas,
      View view,
      Rect dirtyRect,
      float scale,
    ...
    if (CaptureUtils.captureCommon(
        canvas, view, dirtyRect, scale, /* drawWhileDetached= */ drawWhileDetached, observer)) {
      return true;
    }
   ...
   * upfront. And then on a threadpool task we grab a hardware canvas (required to use a RenderNode)
   * and draw it using the hardware accelerated canvas.
   *
  ...
  private boolean captureWithHardwareDraw(
    ...
    try (TraceEvent e = TraceEvent.scoped("HardwareDraw:captureWithHardwareDraw")) {
      ...
      if (currentState.mRequestNewDraw && !dirtyRect.isEmpty()) {
        ...
        if (captureSuccess) {
          onDrawInstructionsAvailable(renderNode, currentState, view, scale, onBitmapCapture);
        }
  ...
  private void onDrawInstructionsAvailable(
      ...
      AcceleratedImageReader.State currentState,
      View view,
      float scale,
      Callback<Bitmap> onBitmapCapture) {
