...
public class ColorUtils {
  ...
  /**
   * Calculates the contrast between the given color and white, using the algorithm provided by the
   * WCAG v2 in http://www.w3.org/TR/WCAG20/#contrast-ratiodef.
   */
  ...
  /**
   * Check whether lighter or darker foreground elements (i.e. text, drawables etc.) should be used
   * depending on the given background color.
   *
  ...
  public static @ColorInt int blendColorsMultiply(
    ...
    int resultGreen = Math.round(MathUtils.interpolate(fromGreen, toGreen, fraction) / resultAlpha);

    int fromBlue = Color.blue(from) * fromAlpha;
    int toBlue = Color.blue(to) * toAlpha;
    int resultBlue = Math.round(MathUtils.interpolate(fromBlue, toBlue, fraction) / resultAlpha);
