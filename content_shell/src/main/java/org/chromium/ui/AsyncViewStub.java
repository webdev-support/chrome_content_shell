...
public class AsyncViewStub extends View implements AsyncLayoutInflater.OnInflateFinishedListener {
  ...
  /**
   * Starts background inflation for the stub, the AsyncViewStub must be attached to the window (ie
   * have a parent) before you call inflate on it. Must be called on the UI thread.
   */
  public void inflate() {
  ...
  /**
   * This should only be used by {@link AsyncViewProvider}, use {@link AsyncViewProvider#whenLoaded}
   * instead.
   *
   * <p>Adds listener that gets called once the view is inflated and added to the view hierarchy.
   * The listeners are called on the UI thread. This method can only be called on the UI thread.
   *
  ...
  /**
   * Sets whether the view should be inflated on a background thread or the UI thread (the default).
   * This method should not be called after the view has been inflated.
   *
