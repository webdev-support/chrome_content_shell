...
public class UiUtils {
  ...
  /**
   * A static map of manufacturers to the version where theming Android UI is completely supported.
   * If there is no entry, it means the manufacturer supports theming at the same version Android
   * did.
   */
   ...
   *     will be scaled to ensure the maximum width or height is equal to or less than this. Any
   *     value <= 0, will result in no scaling.
  ...
  public static Bitmap generateScaledScreenshot(
    ...
    try {
      ...
      if (originalBitmap != null) {
        ...
        canvas.scale((float) (newWidth / originalWidth), (float) (newHeight / originalHeight));
  ...
  /**
   * Get a directory for the image capture operation. For devices with JB MR2 or latter android
   * versions, the directory is IMAGE_FILE_PATH directory. For ICS devices, the directory is
  ...
  public static File getDirectoryForImageCapture(Context context) throws IOException {
    ...
    try {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
        ...
        path =
            new File(externalDataDir.getAbsolutePath() + File.separator + EXTERNAL_IMAGE_FILE_PATH);
  ...
  /**
   * Creates a {@link Typeface} that represents medium-weighted text. This function returns Roboto
   * Medium when it is available (Lollipop and up) and Roboto Bold where it isn't.
   *
  ...
  /**
   * @return Whether the support for theming on a particular device has been completely disabled due
   *     to lack of support by the OEM.
   */
  ...
  /**
   * Sets the navigation bar icons to dark or light. Note that this is only valid for Android O+.
   *
