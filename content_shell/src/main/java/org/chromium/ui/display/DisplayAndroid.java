...
/**
 * Chromium's object for android.view.Display. Instances of this object should be obtained from
 * WindowAndroid. This class is designed to avoid leaks. It is ok to hold a strong ref of this class
 * from anywhere, as long as the corresponding WindowAndroids are destroyed. The observers are held
 * weakly so to not lead to leaks.
 */
public class DisplayAndroid {
  ...
  /**
   * Get the non-multi-display DisplayAndroid for the given context. It's safe to call this with any
   * type of context, including the Application.
   *
  ...
  /**
   * Max luminance HDR content can display, represented as a multiple of the SDR white luminance (so
   * a display that is incapable of HDR would have a value of 1.0).
   */
  // Package private only because no client needs to access this from java.
  ...
  /**
   * Add observer. Note repeat observers will be called only one. Observers are held only weakly by
   * Display.
   */
  ...
  protected void update(
    ...
    boolean isDisplayWideColorGamutChanged =
        isDisplayWideColorGamut != null && mIsDisplayWideColorGamut != isDisplayWideColorGamut;
