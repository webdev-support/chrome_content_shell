...
/* package */ class PhysicalDisplayAndroid extends DisplayAndroid {
  ...
  private static boolean hasForcedDIPScale() {
    if (sForcedDIPScale == null) {
      String forcedScaleAsString =
          CommandLine.getInstance().getSwitchValue(DisplaySwitches.FORCE_DEVICE_SCALE_FACTOR);
  ...
  /* package */ PhysicalDisplayAndroid(Display display) {
    ...
    if (USE_CONFIGURATION) {
      ...
      assert display.getDisplayId() == ApiHelperForR.getDisplay(mWindowContext).getDisplayId();
      mComponentCallbacks =
  ...
  /* package */ void updateFromDisplay(Display display) {
    ...
    updateCommon(size, displayMetrics.density, displayMetrics.xdpi, displayMetrics.ydpi, display);
  }
