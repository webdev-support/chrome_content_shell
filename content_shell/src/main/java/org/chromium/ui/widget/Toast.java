 ...
 *
 * <p>Can (and should) also be used for Chromium-related additions and extensions.
 *
 * <p>When {@link ToastManager} is enabled, priority {@code HIGH} can be used when it is critical
 * that the toast be shown sooner than those of priority {@code NORMAL} queued for their turn. See
 * {@link ToastManager} for more details.
 */
public class Toast {
  ...
  private void anchor(Context context, View anchoredView) {
    ...
    final int yOffset = viewOnTopHalf ? screenPos[1] + height / 2 : screenPos[1] - height * 3 / 2;
   ...
   */
  public static Toast makeTextWithPriority(Context context, int resId, int duration, int priority) {
  ...
  public static boolean showAnchoredToast(
    ...
    return new Builder(context).withAnchoredView(anchoredView).withText(description).buildAndShow();
  }
