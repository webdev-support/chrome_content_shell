 ...
 *
 * <ul>
 *   <li>Queues the requested toasts and shows them one by one in the order of the requested time if
 *       they have the same priority.
 *   <li>Shows the toast with high priority ahead of other queued normal priority ones.
 *   <li>Does not show the requested one again if it is already in the queue or currently showing.
 *       Toasts of same text content are regarded as duplicated.
 * </ul>
 */
@JNINamespace("ui")
public class ToastManager {
  ...
  private class ToastEventPreR implements ToastEvent {
    ...
    public void onShow(Toast toast) {
      int durationMs =
          (mToast.getDuration() == Toast.LENGTH_SHORT) ? DURATION_SHORT_MS : DURATION_LONG_MS;
  ...
  /**
   * Resets ToastManager state to initial state. Cancels the current toast if present, and clears
   * the queue. This prevernts a test running a toast from interfering another one.
   */
