...
public class AnchoredPopupWindow implements OnTouchListener, RectProvider.Observer {
  ...
  public @interface VerticalOrientation {
    /**
     * Vertically position to whichever side of the anchor has more available space. The popup will
     * be sized to ensure it fits on screen.
     */
  ...
  public AnchoredPopupWindow(
    ...
    mViewportRectProvider =
        viewportRectProvider != null ? viewportRectProvider : new RootViewRectProvider(mRootView);
  ...
  /**
   * @param dismiss Whether or not to dismiss this popup when the screen is tapped. This will happen
   *     for both taps inside and outside the popup except when a tap is handled by child views. The
  ...
  /**
   * If set to true, popup will be notified when an outside touch happens. It is not the equivalent
   * of closing the popup on all touch events. The user can still interact with the popup by sending
   * inside touch events. If set to false, the popup won't be notified about the outside touch
   * event.
   *
   * @param touchable Whether or not to notify the popup when an outside touch happens. The default
   *     is {@code false}.
   */
  ...
  /**
   * Sets the preferred horizontal orientation of the popup with respect to the anchor Rect such as
   * centered with respect to the anchor. This should be called before the popup is shown.
   *
  ...
  /**
   * Sets whether the popup should horizontally overlap the anchor {@link Rect}. Defaults to false.
   ...
   * {@link #setVerticalOverlapAnchor} and {@link #setHorizontalOverlapAnchor}. (e.g. Side of anchor
   * is too narrow to make it not horizontally smartAnchor).
   *
  ...
  /**
   * Causes this popup to position/size itself. The calculations will happen even if the popup isn't
   * visible.
   */
   ...
   * @param preferredVerticalOrientation Value set by {@link #setPreferredVerticalOrientation(int)}.
   * @param currentPositionBelow Whether the currently shown popup window is presented below
   *     anchored rect.
   * @param currentPositionToLeft Whether the currently shown popup window is presented to the left
   *     of anchored rect.
   * @param preferCurrentOrientation Whether prefer to reserve the popup orientation. If this set to
   *     true, popup window will prefer to show below / to left of anchored window the same way as
   *     |currentPositionBelow| and |currentPositionToLeft|.
  ...
  static PopupSpec calculatePopupWindowSpec(
    ...
    final int maxContentWidth = getMaxContentWidth(maxWidthPx, rootViewWidth, marginPx, paddingX);
    final int widthSpec =
  ...
  public boolean onTouch(View v, MotionEvent event) {
    boolean touchInterceptedByClient = mTouchListener != null && mTouchListener.onTouch(v, event);
    ...
    if (mDismissOnTouchInteraction) {
      ...
      boolean touchInterceptedByChild =
          !touchInterceptedByClient && mPopupWindow.getContentView().dispatchTouchEvent(event);
   ...
   */
  @VisibleForTesting
  static @StyleRes int calculateAnimationStyle(boolean isPositionBelow, boolean isPositionToLeft) {
    if (isPositionToLeft) {
  ...
  void showPopupWindow() {
    if (mAnimateFromAnchor && mAnimationStyleId == 0) {
      int animationStyle =
          calculateAnimationStyle(mPopupSpec.isPositionBelow, mPopupSpec.isPositionToLeft);
