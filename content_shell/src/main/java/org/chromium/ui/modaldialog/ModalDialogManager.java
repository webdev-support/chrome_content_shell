...
public class ModalDialogManager {
  ...
  public abstract static class Presenter {
    ...
    /**
     * @param model The dialog model that's currently showing in this presenter. If null, no dialog
  ...
  /**
   * This signifies the priority of the dialog. A priority of the dialog influences which dialog
   * will be shown or hidden if there's more than one dialog in the queue of dialogs in {@link
  ...
  /**
   * The list of suspended types of dialogs. The dialogs of types in the list will be suspended from
   * showing and will only be shown after {@link #resumeType(int)} is called.
   */
  ...
  /**
   * The type of the current dialog. This can be different from the type of the current {@link
   * Presenter} if there is no registered presenter for this type.
   */
   ...
   */
  public ModalDialogManager(@NonNull Presenter defaultPresenter, @ModalDialogType int defaultType) {
    mDefaultPresenter = defaultPresenter;
    registerPresenter(defaultPresenter, defaultType);

    mTokenHolders.put(
        ModalDialogType.APP, new TokenHolder(() -> resumeTypeInternal(ModalDialogType.APP)));
    mTokenHolders.put(
        ModalDialogType.TAB, new TokenHolder(() -> resumeTypeInternal(ModalDialogType.TAB)));
  }
  ...
  /**
   * Register a {@link Presenter} that shows a specific type of dialog. Note that only one presenter
   ...
   * dialog will be added to the top of the pending list of its type, otherwise it will be added to
   * the end. The priority of the specified dialog is inferred from the type of the dialog.
   *
   * @param model The dialog model to be shown or added to pending list.
   * @param dialogType The type of the dialog to be shown.
   * @param showAsNext Whether the specified dialog should be set highest priority of its type.
   */
  public void showDialog(PropertyModel model, @ModalDialogType int dialogType, boolean showAsNext) {
   ...
   * the pending dialog list. If the dialog is currently being dismissed this function does nothing.
   *
   ...
   *
   * <p>These dialogs will be prevented from showing unless {@link #resumeType(int, int)} is called
  ...
  /**
   * Resume the specified type of dialogs after suspension. This method does not resume showing the
   * dialog until after all held tokens are released.
   *
  ...
  private void suspendCurrentDialog() {
    ...
    mCurrentPresenter.setDialogModel(null, null);
    mCurrentPresenter = null;
    mPendingDialogContainer.put(mCurrentType, mCurrentPriority, dialogView, /* showAsNext= */ true);
  }
