...
public class GURL {
   ...
   *
   * <p>This function is public so that GURL-related usage like the UrlFormatter also counts towards
  ...
  public static void ensureNativeInitializedForGURL() {
    ...
    if (ThreadUtils.runningOnUiThread()) {
      ...
      RecordHistogram.recordTimesHistogram(
          "Startup.Android.GURLEnsureMainDexInitialized", SystemClock.elapsedRealtime() - time);
   ...
   *
   * <p>Rules for replacement: 1. If a `clear*` boolean param is true, the component will be removed
   * from the result. 2. Otherwise if the corresponding string param is non-null, its value will be
   * used to replace the component. 3. If the string is null and the `clear*` boolean is false, the
   * component will not be modified.
   *
   ...
   *
   * <p>Note that a serialized GURL should only be used internally to Chrome, and should *never* be
  ...
  /**
   * Returns the instance of {@link Natives}. The Robolectric Shadow intercepts invocations of this
   * method.
   *
   * <p>Unlike {@code GURLJni.TEST_HOOKS.setInstanceForTesting}, shadowing this method doesn't rely
   * on tests correctly cleaning up global state.
   */
  ...
  interface Natives {
    ...
    /**
     * Reconstructs the native GURL for this Java GURL and initializes |result| with the result of
     * ReplaceComponents.
     */
