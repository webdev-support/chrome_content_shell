 ...
 *
 * <p>While ContentView is a ViewGroup, the only place that should add children is
 * ViewAndroidDelegate, and only for cases that WebContentsAccessibility handles (such as anchoring
 * popups). This is because the accessibility support provided by WebContentsAccessibility ignores
 * all child views. In other words, any children added to this are *not* accessible.
 */
public class ContentView extends FrameLayout
  ...
  protected WebContentsAccessibility getWebContentsAccessibility() {
    return webContentsAttached() ? WebContentsAccessibility.fromWebContents(mWebContents) : null;
  }
   ...
   * KeepScreenOn flag. If deferring is being turned off, super.setKeepScreenOn will be called with
   * the latest value passed to setKeepScreenOn.
   */
  ...
  /**
   * Set {@link EventOffsetHandler} used to handle drag event offsets. Offsets are provided if the
   * content view is has a different coordinate base than the physical screen (e.g. top browser
   * control).
   *
  ...
  /**
   * Currently the ContentView scrolling happens in the native side. In the Java view system, it is
   * always pinned at (0, 0). scrollBy() and scrollTo() are overridden, so that View's mScrollX and
   * mScrollY will be unchanged at (0, 0). This is critical for drawing ContentView correctly.
   */
  @Override
