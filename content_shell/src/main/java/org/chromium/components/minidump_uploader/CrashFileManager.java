 ...
 * minidump files and shepherding them through a state machine represented by the file names. 1.
 * Minidumps are read from Crashpad's CrashReportDatabase and re-written as MIME files in the "Crash
 * Reports" directory as foo.dmpNNNNN where NNNNN is the PID (process id) of the crashing process.
 * 2. foo.dmpNNNNN.try0 is a minidump file with recent logcat output attached to it; or a file for
 * which logcat output has been intentionally omitted. Notably, Webview-generated minidumps do not
 * include logcat output. 3. foo.dmpNNNNN.tryM for M > 0 is a minidump file that's been attempted to
 * be uploaded to the crash server, but for which M upload attempts have failed. 4. foo.upNNNNN.tryM
 * names a successfully uploaded file. 5. foo.skippedNNNNN.tryM names for a file whose upload was
 * skipped. An upload may be skipped, for example, if the user has not consented to uploading crash
 * reports. These files are marked as skipped rather than deleted immediately to allow the user to
 * manually initiate an upload. 6. foo.forcedNNNNN.tryM names a file that the user has manually
 * requested to upload. 7. foo.tmp is a temporary file.
 */
public class CrashFileManager {
  ...
  private static final Pattern MINIDUMP_SANS_LOGCAT_PATTERN = Pattern.compile("\\.dmp([0-9]*)\\z");
   ...
   * logcat output has been intentionally skipped for that minidump. However, a return value of true
   * means that the file definitely lacks logcat output.
   *
   ...
   * logcat extraction fails or is otherwise intentionally skipped. An equivalent operation is done
   * when extraction succeeds; but since the logcat output needs to be included in the uploaded
   * data, more than a simple rename is needed.
   *
  ...
  protected static String filenameWithForcedUploadState(String filename) {
    ...
    if (numTried > 0) {
      filename =
          filename.replace(UPLOAD_ATTEMPT_DELIMITER + numTried, UPLOAD_ATTEMPT_DELIMITER + 0);
    }
  ...
  /**
   * Marks a crash dump file's upload being skipped. An upload might be skipped due to lack of user
   * consent, or due to this client being excluded from the sample of clients reporting crashes.
   *
  ...
  public static void markUploadSkipped(File crashDumpFile) {
    CrashFileManager.renameCrashDumpFollowingUpload(crashDumpFile, UPLOAD_SKIPPED_MINIDUMP_SUFFIX);
  }

  /**
   * Renames a crash dump file. However, if renaming fails, attempts to delete the file immediately.
   */
  ...
  /**
   * Returns the most recent minidump without a logcat for a given pid, or null if no such minidump
   * exists. This method begins by reading all minidumps from Crashpad's database and rewriting them
   * as MIME files in the Crash Reports directory.
   */
  public File getMinidumpSansLogcatForPid(int pid) {
    importCrashpadMinidumps();
    File[] foundFiles = listCrashFiles(Pattern.compile("\\.dmp" + Integer.toString(pid) + "\\z"));
   ...
   * time stamp. This method begins by reading all minidumps from Crashpad's database and rewriting
   * them as MIME files in the Crash Reports directory. Note: This method does not provide an "if
   * and only if" test: it may return some files that lack logcat output, if logcat output has been
   * intentionally skipped for those minidumps. However, any files returned definitely lack logcat
   * output.
   */
  ...
  /**
   * Returns all minidump files currently in the Crash Reports directory that definitely do not have
   * logcat output, sorted by modification time stamp. Note: This method does not provide an "if and
   * only if" test: it may return some files that lack logcat output, if logcat outpuy has been
   * intentionally skipped for those minidumps. However, any files returned definitely lack logcat
   * output.
   */
  ...
  public File[] getMinidumpsReadyForUpload(int maxTries) {
    return getFilesBelowMaxTries(listCrashFiles(MINIDUMP_ALL_READY_FOR_UPLOAD_PATTERN), maxTries);
  }
  ...
  /**
   * Returns minidump files that are forced to be uploaded by the user, sorted by modification time
   * stamp.
   */
  ...
  /**
   * Delete the oldest minidump if we have reached our threshold on the number of minidumps to store
   * (either per-app, or globally).
   *
  ...
  private void enforceMinidumpStorageRestrictions(int uid) {
    ...
    if (minidumpFilesWithCurrentUid.size() >= MAX_CRASH_REPORTS_TO_UPLOAD_PER_UID) {
      ...
      File oldestFile = minidumpFilesWithCurrentUid.get(minidumpFilesWithCurrentUid.size() - 1);
