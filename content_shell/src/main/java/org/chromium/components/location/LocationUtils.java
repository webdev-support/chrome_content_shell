 ...
 *
 * <p>This class should be used only on the UI thread.
 */
public class LocationUtils {
  ...
  private boolean hasPermission(String name) {
    ...
    return ApiCompatibilityUtils.checkPermission(context, name, Process.myPid(), Process.myUid())
   ...
   *
   * <p>Callers should check both hasAndroidLocationPermission() and
   * isSystemLocationSettingEnabled() to determine if Chromium's location requests will return
   * results.
   */
  ...
  /**
   * Returns whether location services are enabled system-wide, i.e. whether any application is able
   * to access location.
   */
   ...
   *
   * <p>In particular, returns false if the system location setting is already enabled or if some of
   * the features required to trigger a system location setting prompt are not available.
   */
   ...
   *
   * <p>The callback is guaranteed to be called unless the user never replies to the prompt dialog,
   * which in practice happens very infrequently since the dialog is modal.
   *
   * <p>TODO(crbug/730711): Add back @LocationSettingsDialogOutcome to the callback when type
  ...
  /**
   * Call this to use a different subclass of LocationUtils throughout the program. This can be used
   * by embedders in addition to tests.
   */
