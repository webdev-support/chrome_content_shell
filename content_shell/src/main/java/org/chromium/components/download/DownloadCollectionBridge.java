...
public class DownloadCollectionBridge {
  ...
  /**
   * Sets the DownloadDelegate to be used for utility methods. TODO(qinmin): remove this method once
   * we moved all the utility methods into components/.
   *
  ...
  public static String publishDownload(final String intermediateUri) {
    ...
    try {
      cursor =
          resolver.query(
              Uri.parse(intermediateUri), new String[] {MediaColumns.MIME_TYPE}, null, null, null);
  ...
  private static int openIntermediateUri(final String intermediateUri) {
    try {
      ContentResolver resolver = ContextUtils.getApplicationContext().getContentResolver();
      ParcelFileDescriptor pfd = resolver.openFileDescriptor(Uri.parse(intermediateUri), "rw");
  ...
  private static Uri createPendingSessionInternal(
      ...
      final String originalUrl,
      final String referrer) {
    PendingParams pendingParams = createPendingParams(fileName, mimeType, originalUrl, referrer);
    pendingParams.setExpirationTime(getNewExpirationTime());
    try {
      return MediaStoreUtils.createPending(ContextUtils.getApplicationContext(), pendingParams);
    } catch (Exception e) {
      return null;
    }
  }
  ...
  private static PendingParams createPendingParams(
    ...
    String newMimeType = sDownloadDelegate.remapGenericMimeType(mimeType, originalUrl, fileName);
  ...
  /**
   * Gets the base name, without extension, from a file name. TODO(qinmin): move this into a common
   * utility class.
   *
   * @param fileName Name of the file.
  ...
  private static @NonNull PendingSession openPendingUri(final String pendingUri) {
    return MediaStoreUtils.openPending(ContextUtils.getApplicationContext(), Uri.parse(pendingUri));
  }
  ...
  private static long getNewExpirationTime() {
    return (System.currentTimeMillis()
            + DownloadCollectionBridge.getExpirationDurationInDays() * DateUtils.DAY_IN_MILLIS)
        / 1000;
  }
  ...
  private static String getDisplayName(final String downloadUri) {
    ...
    try {
      cursor =
          resolver.query(
              Uri.parse(downloadUri), new String[] {MediaColumns.DISPLAY_NAME}, null, null, null);
      if (cursor == null || cursor.getCount() == 0) return null;
      if (cursor.moveToNext()) {
