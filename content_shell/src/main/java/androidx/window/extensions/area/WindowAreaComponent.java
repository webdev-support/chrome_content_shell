...
public interface WindowAreaComponent {
  ...
  /**
   * WindowArea status constant to signify that the feature is unsupported on this device. Could be
   * due to the device not supporting that specific feature.
   *
   * <p>Since {@link WindowExtensions#VENDOR_API_LEVEL_2}
   */
   ...
   *
   * <p>Since {@link WindowExtensions#VENDOR_API_LEVEL_3}
   */
  int STATUS_ACTIVE = 3;
  ...
  /**
   * Session state constant to represent that there is currently an active session. The library uses
   * this state to know when a session is created and active. Note that this state is different from
   * SESSION_STATE_CONTENT_VISIBLE, because the presentation content in this state is not visible.
   */
  ...
  /**
   * Adds a listener interested in receiving updates on the RearDisplayStatus of the device. Because
   * this is being called from the OEM provided extensions, the library will post the result of the
   * listener on the executor provided by the developer.
   *
   ...
   *
   * @param consumer interested in receiving updates to WindowAreaStatus. Since {@link
   ...
   *
   * @param consumer no longer interested in receiving updates to WindowAreaStatus Since {@link
   *     WindowExtensions#VENDOR_API_LEVEL_2}
   */
   ...
   * This consumer will receive a constant represented by [WindowAreaSessionState] to represent the
   * state of the current rear display session. We will translate to a more friendly interface in
   * the library.
   *
   ...
   *
   * @param activity to allow that the OEM implementation will use as a base context and to identify
   *     the source display area of the request. The reference to the activity instance must not be
   ...
   *     currently in this mode. Since {@link WindowExtensions#VENDOR_API_LEVEL_2}
   */
  @SuppressWarnings("ExecutorRegistration") // Jetpack will post it on the app-provided executor.
  void startRearDisplaySession(
      @NonNull Activity activity, @NonNull Consumer<@WindowAreaSessionState Integer> consumer);
   ...
   * RearDisplaySession} provided to the developer. Since {@link
   * WindowExtensions#VENDOR_API_LEVEL_2}
   */
  void endRearDisplaySession();

  /**
   * Adds a listener interested in receiving updates on the rear display presentation status of the
   * device. Because this is being called from the OEM provided extensions, the library will post
   * the result of the listener on the executor provided by the developer.
   *
   * <p>The listener provided will receive {@link ExtensionWindowAreaStatus} values that correspond
   * to the current status of the feature.
   *
   * @param consumer interested in receiving updates to {@link ExtensionWindowAreaStatus}. Since
   *     {@link WindowExtensions#VENDOR_API_LEVEL_3}
   */
  ...
  /**
   * Creates and starts a rear display presentation session and sends state updates to the consumer
   * provided. This consumer will receive a constant represented by {@link WindowAreaSessionState}
   * to represent the state of the current rear display session. We will translate to a more
   * friendly interface in the library.
   *
   * <p>Because this is being called from the OEM provided extensions, the library will post the
   * result of the listener on the executor provided by the developer.
   *
   ...
   *     process is currently in this mode. Since {@link WindowExtensions#VENDOR_API_LEVEL_3}
   */
   ...
   * provided. When this is ended, the presented content from the calling {@link Activity} will also
   * be removed from the rear facing display. Because this is being called from the OEM provided
   * extensions, the result of the listener will be posted on the executor provided by the developer
   * at the initial call site.
   *
   * <p>Since {@link WindowExtensions#VENDOR_API_LEVEL_3}
   */
