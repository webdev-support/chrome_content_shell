 ...
 *   <li>Animation background color -- The color of the background during animation of the split
 *       involving this {@code SplitAttributes} object if the animation requires a background
 * </ul>
 *
 ...
 * @see SplitAttributes.LayoutDirection Since {@link
 *     androidx.window.extensions.WindowExtensions#VENDOR_API_LEVEL_2}
 */
public class SplitAttributes {
   ...
   * background color.
   *
   * <p>Only opaque color is supported, so {@code 0} is used as the default. Any other non-opaque
  ...
  public static class SplitType {
    ...
    /**
     * A window split that's based on the ratio of the size of the primary container to the size of
     * the parent window.
     *
     ...
     *
     * <ul>
     *   <li>0.5 -- Primary container occupies half of the parent window; secondary container, the
     *       other half
     *   <li>Greater than 0.5 -- Primary container occupies a larger proportion of the parent window
     *       than the secondary container
     *   <li>Less than 0.5 -- Primary container occupies a smaller proportion of the parent window
     *       than the secondary container
     * </ul>
     */
    public static final class RatioSplitType extends SplitType {
       ...
       *
       * @param ratio The proportion of the parent window occupied by the primary container of the
       *     split. Can be a value in the non-inclusive range (0.0, 1.0). Use {@link
       *     SplitType.ExpandContainersSplitType} to create a split type that occupies the entire
       *     parent window.
       */
      ...
      /**
       * Gets the proportion of the parent window occupied by the primary activity container of the
       * split.
       *
      ...
      /**
       * Creates a split type in which the primary and secondary containers occupy equal portions of
       * the parent window.
       *
       * <p>Serves as the default {@link SplitType} if {@link
       * SplitAttributes.Builder#setSplitType(SplitType)} is not specified.
       *
       * @return A {@code RatioSplitType} in which the activity containers occupy equal portions of
       *     the parent window.
       */
      @NonNull
     ...
     *         <li>The hinge or fold orientation is horizontal, and the task bounds are also split
     *             horizontally (containers are top and bottom)
     *       </ul>
     * </ul>
     *
    ...
    public static final class HingeSplitType extends SplitType {
      @NonNull private final SplitType mFallbackSplitType;

      /**
       * Creates an instance of this {@code HingeSplitType}.
       *
       * @param fallbackSplitType The split type to use if a split based on the device hinge or
       *     separating fold cannot be determined. Can be a {@link RatioSplitType} or {@link
      ...
      /**
       * Returns the fallback {@link SplitType} if a split based on the device hinge or separating
       * fold cannot be determined.
       */
      @NonNull
    ...
    /**
     * A window split in which the primary and secondary activity containers each occupy the entire
     * parent window.
     *
  ...
  public static final class LayoutDirection {
     ...
     *
     * <p>Places the primary container in the left portion of the parent window, and the secondary
     * container in the right portion.
     *
     ...
     * Specifies that the parent bounds are split vertically (side to side).
     *
     * <p>Places the primary container in the right portion of the parent window, and the secondary
     * container in the left portion.
     *
     * <p>A possible return value of {@link SplitType#getLayoutDirection()}.
     */
    // -------------------------
     ...
     * Specifies that the parent bounds are split vertically (side to side).
     *
     * <p>The direction of the primary and secondary containers is deduced from the locale as either
     * {@link #LEFT_TO_RIGHT} or {@link #RIGHT_TO_LEFT}.
     *
     * <p>A possible return value of {@link SplitType#getLayoutDirection()}.
     */
     ...
     *
     * <p>Places the primary container in the top portion of the parent window, and the secondary
     * container in the bottom portion.
     *
     * <p>If the horizontal layout direction is not supported on the device, layout direction falls
     * back to {@link #LOCALE}.
     *
     * <p>A possible return value of {@link SplitType#getLayoutDirection()}.
     */
     ...
     * Specifies that the parent bounds are split horizontally (top and bottom).
     *
     * <p>Places the primary container in the bottom portion of the parent window, and the secondary
     * container in the top portion.
     *
     * <p>If the horizontal layout direction is not supported on the device, layout direction falls
     * back to {@link #LOCALE}.
     *
  ...
  @ColorInt private final int mAnimationBackgroundColor;
   ...
   * @param animationBackgroundColor The {@link ColorInt} to use for the background color during
   *     animation of the split involving this {@code SplitAttributes} object if the animation
  ...
  /**
   * Gets the {@link ColorInt} to use for the background color during the animation of the split
   * involving this {@code SplitAttributes} object.
   *
   * <p>The default is {@link #DEFAULT_ANIMATION_BACKGROUND_COLOR}, which means to use the current
   ...
   * default layout direction is based on locale. - The default animation background color is to use
   * the current theme window background color.
   */
  public static final class Builder {
    @NonNull private SplitType mSplitType = new SplitType.RatioSplitType(0.5f);
    @ExtLayoutDirection private int mLayoutDirection = LOCALE;

    @ColorInt private int mAnimationBackgroundColor = 0;
    ...
    /**
     * Sets the {@link ColorInt} to use for the background during the animation of the split
     * involving this {@code SplitAttributes} object if the animation requires a background.
     *
     * <p>Only opaque color is supported.
     *
     * <p>The default value is {@link #DEFAULT_ANIMATION_BACKGROUND_COLOR}, which means to use the
     * current theme window background color. Any non-opaque animation color will be treated as
     * {@link #DEFAULT_ANIMATION_BACKGROUND_COLOR}.
     *
     * @param color A packed color int of the form {@code AARRGGBB} for the animation background
     *     color.
     * @return This {@code Builder}.
     */
    @NonNull
    ...
    public Builder setAnimationBackgroundColor(@ColorInt int color) {
      ...
      mAnimationBackgroundColor =
          Color.alpha(color) != 255 ? DEFAULT_ANIMATION_BACKGROUND_COLOR : color;
      return this;
    }

    /**
     * Builds a {@link SplitAttributes} instance with the attributes specified by {@link
     * #setSplitType}, {@link #setLayoutDirection}, and {@link #setAnimationBackgroundColor}.
     *
  ...
  public String toString() {
    return SplitAttributes.class.getSimpleName()
        ...
        + mSplitType
        + ", animationBgColor="
        + Integer.toHexString(mAnimationBackgroundColor)
        + "}";
  }
