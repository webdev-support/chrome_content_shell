...
public class SplitPlaceholderRule extends SplitRule {
  @NonNull private final Predicate<Activity> mActivityPredicate;
  @NonNull private final Predicate<Intent> mIntentPredicate;
  @NonNull private final Intent mPlaceholderIntent;
  private final boolean mIsSticky;

  /**
   * Determines what happens with the primary container when the placeholder activity is finished in
  ...
  /**
   * @deprecated Use {@link #getFinishPrimaryWithPlaceholder()} instead starting with {@link
   *     WindowExtensions#VENDOR_API_LEVEL_2}. Only used if {@link
   *     #getFinishPrimaryWithPlaceholder()} can't be called on {@link
  ...
  public static final class Builder {
    @NonNull private final Predicate<Activity> mActivityPredicate;
    @NonNull private final Predicate<Intent> mIntentPredicate;
    @NonNull private final Predicate<WindowMetrics> mParentWindowMetricsPredicate;
    @NonNull private final Intent mPlaceholderIntent;
     ...
     *     {@link WindowExtensions#VENDOR_API_LEVEL_2}. Only used if {@link #Builder(Intent,
     *     Predicate, Predicate, Predicate)} can't be called on {@link
     *     WindowExtensions#VENDOR_API_LEVEL_1}.
     */
    @Deprecated
     ...
     * @param activityPredicate the {@link Predicate} to verify if a given {@link Activity} matches
     *     the rule
     * @param intentPredicate the {@link Predicate} to verify if a given {@link Intent} matches the
     *     rule
     * @param parentWindowMetricsPredicate the {@link Predicate} to verify if the placeholder {@link
     *     Activity} should be launched with the given {@link WindowMetrics} Since {@link
    ...
    /**
     * @deprecated Use {@link #setDefaultSplitAttributes(SplitAttributes)} starting with {@link
     *     WindowExtensions#VENDOR_API_LEVEL_2}. Only used if {@link
     *     #setDefaultSplitAttributes(SplitAttributes)} can't be called on {@link
     *     WindowExtensions#VENDOR_API_LEVEL_1}. {@code splitRatio} will be translated to
     * @link SplitAttributes.SplitType.ExpandContainersSplitType} for value {@code 0.0} and {@code
    ...
    /**
     * See {@link SplitPlaceholderRule#getDefaultSplitAttributes()} for reference. Overrides values
     * if set in {@link #setSplitRatio(float)} and {@link #setLayoutDirection(int)}
     *
     * <p>Since {@link WindowExtensions#VENDOR_API_LEVEL_2}
     */
    @NonNull
    ...
    /**
     * @see SplitPlaceholderRule#getFinishPrimaryWithPlaceholder() Since {@link
     *     WindowExtensions#VENDOR_API_LEVEL_2}
     */
    // TODO(b/238905747): Add api guard for extensions.
    @NonNull
    ...
    /**
     * @see SplitPlaceholderRule#getTag() Since {@link WindowExtensions#VENDOR_API_LEVEL_2}
     */
    @NonNull
