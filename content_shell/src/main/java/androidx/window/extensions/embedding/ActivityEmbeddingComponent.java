...
public interface ActivityEmbeddingComponent {
  ...
  /**
   * Updates the rules of embedding activities that are started in the client process. Since {@link
   * WindowExtensions#VENDOR_API_LEVEL_1}
   */
  void setEmbeddingRules(@NonNull Set<EmbeddingRule> splitRules);

  /**
   * @deprecated Use {@link #setSplitInfoCallback(Consumer)} starting with {@link
   *     WindowExtensions#VENDOR_API_LEVEL_2}. Only used if {@link #setSplitInfoCallback(Consumer)}
   *     can't be called on {@link WindowExtensions#VENDOR_API_LEVEL_1}. Since {@link
   *     WindowExtensions#VENDOR_API_LEVEL_1}
   */
  @Deprecated
   ...
   *
   * @param consumer the callback to notify {@link SplitInfo} list changes Since {@link
   ...
   * java.lang.NoSuchMethodError}. Since {@link WindowExtensions#VENDOR_API_LEVEL_2}
   */
  void clearSplitInfoCallback();

  /**
   * Checks if an activity's' presentation is customized by its or any other process and only
   * occupies a portion of Task bounds. Since {@link WindowExtensions#VENDOR_API_LEVEL_1}
   */
   ...
   *       constraints defined by {@link SplitRule#checkParentMetrics(WindowMetrics)} are satisfied
   ...
   *     exists. Since {@link WindowExtensions#VENDOR_API_LEVEL_2}
   */
   ...
   *
   * @see #setSplitAttributesCalculator(Function) Since {@link WindowExtensions#VENDOR_API_LEVEL_2}
   */
   ...
   * @param token The {@link ActivityStack#getToken()} to represent the {@link ActivityStack} Since
   *     {@link WindowExtensions#VENDOR_API_LEVEL_3}
   */
  @NonNull
  default ActivityOptions setLaunchingActivityStack(
   ...
   *     finished. Since {@link WindowExtensions#VENDOR_API_LEVEL_3}
   */
  ...
  /**
   * Triggers an update of the split attributes for the top split if there is one visible by making
   * extensions invoke the split attributes calculator callback. This method can be used when a
   * change to the split presentation originates from the application state change rather than
   * driven by parent window changes or new activity starts. The call will be ignored if there is no
   * visible split.
   *
   * @see #setSplitAttributesCalculator(Function) Since {@link WindowExtensions#VENDOR_API_LEVEL_3}
   */
   ...
   * @param splitAttributes The {@link SplitAttributes} to apply to the split pair. Since {@link
